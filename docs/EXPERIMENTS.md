...
- Result & verdict: primera batería real ejecutada el 04-sep-2026; 9/9 solicitudes alcanzaron el proveedor, pero Opus rechazó un parámetro obsoleto y cuatro respuestas Haiku agotaron el presupuesto antes de cerrar JSON. Segunda batería: 6/9 completaron y el Consejo bloqueó correctamente la salida demo; tres Haiku siguieron truncados. Se incorporó structured output; su primera compilación reveló que Anthropic no admite `maxItems`/`maxLength`, por lo que esos límites permanecen en prompt y normalización, no en el esquema. Repetición completa pendiente.
