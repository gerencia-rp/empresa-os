...
function rcVivoGanancia(o) {
  ...
  return '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:6px;margin-top:8px;padding:7px 9px;border-radius:8px;background:' + bg + '">'
    + '<span style="font-size:10px;color:var(--txt3,#98a0ae)">GANANCIA PROYECTADA ' + (o.sem_ganancia === 'rojo' ? 'PÉRDIDA' : kitStatusDot('ok')) + tentativo + precioTent + '</span>' + modoSel
...
function rcVivoCard() {
  ...
  const card = (o) => {
    ...
    return `<div class="card" style="min-width:0">
      ...
      <div style="font-size:10px;color:var(--txt3,#98a0ae);margin-top:8px">AVANCE TÉCNICO ${o.metodo === 'ponderado_actividad' ? '$ por actividad' : o.metodo === 'ponderado_etapa' ? 'por etapa' : '(conteo)'} · ${o.pct_tecnico || 0}%${o.tareas_sin_map > 0 && o.metodo === 'conteo' ? ` <span style="color:#fbbf24" title="Tareas del Planner que no cruzan con actividades del Estimador (planner manual o pronóstico sin guardar) — no ponderan hasta mapear">· ${o.tareas_sin_map} sin mapear</span>` : ''}</div>${bar(o.pct_tecnico, 'linear-gradient(90deg,#5c79f0,#3a5be0)')}
      <div style="font-size:10px;color:var(--txt3,#98a0ae)">AVANCE FINANCIERO${o.fuente_presupuesto === 'pronostico' ? ' (presup. del pronóstico)' : ''} · ${o.pct_financiero != null ? o.pct_financiero + '%' : 's/presup'} ${o.presupuesto ? `($${(+o.gasto_real).toLocaleString('en-US')} de $${(+o.presupuesto).toLocaleString('en-US')})` : ''}</div>${bar(o.pct_financiero, o.sobrecosto_vs_tecnico ? 'linear-gradient(90deg,#fbbf24,#ff6b6b)' : 'linear-gradient(90deg,#4ade9e,#5c79f0)')}
      <div style="font-size:10px;color:var(--txt3,#98a0ae)">AVANCE TEMPORAL · ${o.pct_temporal != null ? o.pct_temporal + '%' : 's/cronograma'}</div>${bar(o.pct_temporal, 'linear-gradient(90deg,#98a0ae,#8b93a1)')}
