...
function rcObraDataset() {
  return (RC.obras || []).map(o => {
    ...
    return {
      ...
      rentabilidad: rcRentPct(o), rentabilidad_con_draws: o.rentabilidad != null ? +o.rentabilidad : null,
...
function rcCompute() {
  ...
  const evr = fin.filter(o => (+o.presupuesto_interno || 0) > 0 && rcFin(o).costoReal > 0).map(o => {
    ...
    return { address: rcShort(o.address), lider: (o.lider || '—'), est, real, devAbs: real - est, devPct: Math.round((real - est) / est * 100), devDias: o.retraso_dias != null ? +o.retraso_dias : null, rent: rcRentPct(o) };
  });
  ...
  // B9 (auditoría 13-jul): rentabilidad = margen PONDERADO Σutilidad/Σvalor, excluyendo denominadores ≤0
  // (JAMÁS promediar la columna % — fracción de Airtable, y el promedio simple infla outliers).
  // Métrica LIMPIA: Σ utilidad_remodelacion / Σ valor_cliente (misma definición que rentabilidad_remodelacion, agregada).
  const _rentOK = fin.filter(o => (+o.valor_cliente || 0) > 0 && rcUtil(o) != null);
  const rentProm = _rentOK.length ? +((100 * _rentOK.reduce((s, o) => s + rcUtil(o), 0)) / _rentOK.reduce((s, o) => s + (+o.valor_cliente || 0), 0)).toFixed(1) : 0;
...
function rcSecCommand(c) {
  ...
  return rcHeader('Command Center', `${c.obras.length} obras · ${c.fin.length} finalizadas · ${c.activas.length} en curso — capital, ganancia realizada y pipeline.`) + `
    ...
    <div class="grid kpis" style="grid-template-columns:repeat(3,minmax(0,1fr))">
      <div class="card kpi"><div class="lab">Servicio (Remodelación)</div><div class="big ${c.gananciaHist>=0?'up glow':'down'}">${RC_K(c.gananciaHist)}</div><div class="meta">utilidad limpia · ${c.fin.length} finalizadas · margen ${c.margenHist}% · con Draws: ${RC_K(c.gananciaConDraws)}</div></div>
      <div class="card kpi"><div class="lab">Costo Fix & Flip</div><div class="big">${RC_K(c.costoFFHist)}</div><div class="meta">intereses + servicios públicos + muebles del flip</div></div>
  ...
  return rcHeader('Gestión (EVM)', `CPI/SPI por casa (subconjunto de las ${c.activas.length} en curso que ya tienen fechas y costo cargados), avance físico vs planeado, y agregados de calibración listos para el Estimador.`) + `
    ...
    <div class="grid" style="grid-template-columns:1fr">
      <div class="card"><div class="chart-h"><div class="t">Control de presupuesto por casa</div><div class="k">material (pagos) + MO (horas×rate) · alerta > presup +${RC.sobrecostoPct != null ? RC.sobrecostoPct : 10}%</div></div>
        ...
        ${(() => { const map = {}; (RC.ledger || []).forEach(r => { const wk = (r.worker || '').trim(); if (!map[wk]) map[wk] = { w: wk, horas: 0, dev: 0, pag: 0, deuda: 0, casas: [] }; const m2 = map[wk]; m2.horas += +r.horas || 0; m2.dev += +r.devengado || 0; m2.pag += +r.pagado || 0; m2.deuda += +r.deuda || 0; if (+r.deuda > 100) m2.casas.push(rcShort(r.casa) + ' ' + RC_M(+r.deuda)); }); const tot = Object.values(map).filter(x => Math.abs(x.deuda) > 100).sort((x, y) => y.deuda - x.deuda); const deudaTotal = tot.reduce((s2, x) => s2 + Math.max(0, x.deuda), 0); return `<div class="krow"><span><b>DEUDA TOTAL</b></span><b class="down">${RC_M(deudaTotal)}</b></div>` + tot.slice(0, 8).map(x => `<div class="krow"><span>${RC_E(x.w)} <span style="opacity:.5;font-size:10px">(${Math.round(x.horas)}h · ${x.casas.slice(0, 2).join(', ')})</span></span><b class="${x.deuda > 0 ? 'down' : 'up'}">${RC_M(x.deuda)}</b></div>`).join(''); })()}
...
function rcSecNomina(c) {
  const led = RC.ledger || [];
  const porW = {};
  led.forEach(r => { const wk = (r.worker || '').trim(); if (!porW[wk]) porW[wk] = { w: wk, horas: 0, dev: 0, pag: 0, deuda: 0, casas: [] }; const m = porW[wk]; m.horas += +r.horas || 0; m.dev += +r.devengado || 0; m.pag += +r.pagado || 0; m.deuda += +r.deuda || 0; if (Math.abs(+r.deuda) > 50) m.casas.push({ casa: r.casa, deuda: +r.deuda, horas: +r.horas, ult: r.ultima_fecha }); });
  ...
  const filaW = x => {
    ...
    // Item 52: pagado >> devengado es imposible (sobrepago o datos rotos) → marcarlo, no esconderlo.
    const sobrepago = Math.max(0, x.pag - x.dev);
    const anomalo = x.dev > 0 && x.pag > x.dev * 1.2;
    const flag = anomalo ? ` <span class="ff-dq ff-dq-rev" title="Pagado (${RC_M(x.pag)}) mayor que devengado (${RC_M(x.dev)}) — sobrepago ${RC_M(sobrepago)} o dato roto (horas/rate). Revisar en Airtable.">⚠ pagado &gt; devengado</span>` : '';
    return `<tr style="cursor:pointer" onclick="RC._nomAbierto['${RC_E(x.w).replace(/'/g, '')}']=!RC._nomAbierto['${RC_E(x.w).replace(/'/g, '')}'];rcRender()"><td><b>${RC_E(x.w)}</b> <span style="opacity:.4">${open ? '▾' : '▸'}</span>${flag}</td><td style="text-align:right">${Math.round(x.horas)}h</td><td style="text-align:right">${RC_M(x.dev)}</td><td style="text-align:right">${RC_M(x.pag)}</td><td style="text-align:right"><b class="${x.deuda > 0 ? 'down' : 'up'}">${RC_M(x.deuda)}</b></td></tr>${det}`;
  };
  ...
  return `${rcHeader('Nómina y Pagos', 'Structure One', `deuda neta ${RC_M(deudaTotal)} · ${workers.filter(x => x.deuda > 100).length} trabajador(es) con saldo`)}
    ...
    <div class="grid row2" style="margin-top:14px">
      <div class="card"><div class="chart-h"><div class="t">Deuda por trabajador</div><div class="k">click para detalle por casa · grano: trabajador×casa×día</div></div>
        ...
        ${workers.filter(x => Math.abs(x.deuda) > 100 || (x.dev > 0 && x.pag > x.dev * 1.2)).map(filaW).join('') || '<tr><td colspan="5" style="padding:12px;color:#48d69c">Sin deudas ✓</td></tr>'}</tbody></table></div>
