(function () {
  ...
  function renderPublicResearch() {
    ...
    return `<section class="research-stack">
      ...
      <div class="research-profiles">${research.profiles.map(profile => `<a class="research-profile ui-card" href="${esc(profile.url)}" target="_blank" rel="noopener noreferrer"><span>${esc(profile.platform)}</span><strong>${esc(profile.handle)}</strong><div>${profile.status === 'available' ? `<b>${formatNumber(profile.followers)}</b> seguidores · <b>${formatNumber(profile.posts)}</b> piezas${profile.likes ? ` · <b>${formatNumber(profile.likes)}</b> me gusta` : ''}` : '<b>No disponible en esta lectura</b>'}</div><small>${esc(profile.source)}</small></a>`).join('')}</div>
