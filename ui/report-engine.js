...
function reportOpen(cfg) {
  ...
  const estadoC = e => e === 'ok' ? '#1f7a4d' : e === 'neg' ? '#b3372f' : e === 'warn' ? '#8a6400' : '#211e17';
  ...
  const render = s => {
    if (s.t === 'kpis') return '<div class="kgrid" style="grid-template-columns:repeat(' + Math.min(4, s.items.length) + ',1fr)">'
      + s.items.map(k => '<div class="k"><div class="l">' + E(k.lab) + '</div><div class="v" style="color:' + estadoC(k.estado) + '">' + (k.val == null || k.val === '' ? '<span style="color:#a89f8f;font-size:13px">sin dato</span>' : k.val) + '</div>' + (k.sub ? '<div class="s">' + k.sub + '</div>' : '') + '</div>').join('') + '</div>';
    if (s.t === 'tabla') return sec(s.titulo, '<table><thead><tr>' + s.cols.map(c => '<th>' + E(c) + '</th>').join('') + '</tr></thead><tbody>'
      + s.rows.map(r => '<tr>' + r.map(c => '<td>' + (c == null ? '<span style="color:#a89f8f">—</span>' : c) + '</td>').join('') + '</tr>').join('') + '</tbody></table>'
      + (s.nota ? '<div class="nota">' + s.nota + '</div>' : ''));
    if (s.t === 'filas') return sec(s.titulo, s.filas.map(f => '<div class="row' + (f[2] ? ' ' + f[2] : '') + '"><span>' + f[0] + '</span><b>' + (f[1] == null ? '<span style="color:#a89f8f;font-weight:400">sin dato</span>' : f[1]) + '</b></div>').join(''));
    if (s.t === 'texto') return sec(s.titulo, '<div style="font-size:12.5px;line-height:1.55;color:#5f594c">' + s.html + '</div>');
    return '';
  };
  const html = '<!doctype html><html><head><meta charset="utf-8"><link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,500;9..144,600;9..144,700&display=swap" rel="stylesheet"><title>' + E(cfg.titulo) + ' · Rental Profits</title><style>'
    + 'body{font-family:-apple-system,"Segoe UI",Inter,sans-serif;color:#211e17;margin:0 auto;padding:34px;max-width:780px;line-height:1.45;background:#fff}'
    + '.top{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #45936c;padding-bottom:13px;margin-bottom:4px}'
    + '.logo{font-size:20px;font-weight:800;color:#275c43;font-family:Fraunces,Georgia,serif}.logo em{font-style:normal;color:#45936c}.logo span{display:block;font-size:9px;letter-spacing:2.2px;color:#756c5c;font-weight:700;margin-top:2px}'
    + 'h1{font-size:21px;margin:14px 0 2px;font-family:Fraunces,Georgia,serif}.sub{font-size:12px;color:#756c5c}'
    + '.badge{display:inline-block;font-size:11px;font-weight:800;color:#fff;background:#2f6b4f;padding:3px 12px;border-radius:18px;margin-top:8px}'
    + '.kgrid{display:grid;gap:9px;margin:14px 0}'
    + '.k{border:1px solid #e8e3d9;border-radius:10px;padding:10px 12px;background:#faf8f4}'
    + '.k .l{font-size:8.5px;text-transform:uppercase;letter-spacing:.7px;color:#756c5c;font-weight:800}'
    + '.k .v{font-size:18px;font-weight:800;margin-top:2px;font-variant-numeric:tabular-nums}.k .s{font-size:9.5px;color:#756c5c;margin-top:1px}'
    + '.sec{margin-top:14px;border:1px solid #e8e3d9;border-radius:11px;padding:13px 15px;page-break-inside:avoid}'
    + '.st{font-size:10px;text-transform:uppercase;letter-spacing:1px;font-weight:800;color:#275c43;margin-bottom:7px}'
    + 'table{border-collapse:collapse;width:100%;font-size:11.5px}th{text-align:left;font-size:9px;text-transform:uppercase;color:#756c5c;padding:4px 8px;border-bottom:2px solid #e8e3d9}td{padding:5px 8px;border-bottom:1px solid #f7f5f0}'
    + '.b-ok{background:#e7f8f0;color:#1f7a4d;font-size:9.5px;font-weight:800;padding:1px 8px;border-radius:12px}.b-warn{background:#f6eed6;color:#8a6400;font-size:9.5px;font-weight:800;padding:1px 8px;border-radius:12px}.b-neg{background:#fdeaee;color:#b3372f;font-size:9.5px;font-weight:800;padding:1px 8px;border-radius:12px}'
    + '.row{display:flex;justify-content:space-between;font-size:12px;padding:4.5px 0;border-bottom:1px dashed #f7f5f0}.row b{font-variant-numeric:tabular-nums}.row.neg b{color:#b3372f}.row.big{font-size:13.5px}.row.big b{font-size:14px}'
    + '.nota{font-size:10px;color:#756c5c;margin-top:7px}'
    + '.foot{margin-top:20px;padding-top:11px;border-top:1px solid #e8e3d9;font-size:9.5px;color:#a89f8f;display:flex;justify-content:space-between;gap:10px}'
    + '.btn{position:fixed;top:12px;right:12px;padding:9px 18px;background:linear-gradient(135deg,#45936c,#2f6b4f);color:#fff;border:none;border-radius:10px;cursor:pointer;font-weight:800;font-size:13px}'
    ...
    + '<div style="text-align:right;font-size:11px;color:#756c5c">' + fecha + '</div></div>'
    + '<h1>' + E(cfg.titulo) + '</h1>'
