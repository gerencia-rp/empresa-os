...
## 🎯 Estado (18 Sep 2026 — 🚢 DEPLOY del control de accesos + el mito del "dominio desviado") · EN VIVO

- 🚢 **El dominio NUNCA estuvo desviado ni clavado.** `empresa-os.vercel.app` servía `dpl_F4X9dy…` creado **2026-09-05T00:17:55Z**, y el bundle vivo `76e6743dd493` decía `builtAt 2026-09-05T00:18:16Z`: casaban al minuto. **No había despliegues de producción más nuevos** — desde el 5-sep no se pusheó nada. La pantalla de Accesos no estaba viva porque **el código no estaba en ninguna rama**: quedó sin commitear en el working tree local.
- 🔎 **`commit:"local"` tiene diagnóstico exacto**: `scripts/build.mjs` → `gitCommit()` usa `VERCEL_GIT_COMMIT_SHA` y si no está cae a `git rev-parse`, devolviendo `"local"` solo cuando el build corre **sin git y sin las env de Git de Vercel** = un `vercel --prod` desde una máquina. Ese despliegue lo hizo el actor **`codex` desde la rama `codex/growth-command`**. ⚠ **Un `vercel --prod` por CLI toma el dominio de producción aunque el Production Branch sea `main`** — saltea el flujo de GitHub. Si un build vivo dice `commit:"local"`, la producción NO salió de `main`.
- 🔀 **`codex/growth-command` era `main` + 4 commits Y ESOS 4 ESTABAN EN PRODUCCIÓN** (desde el 5-sep): `growth/*`, `growth-command.html` y ~230 líneas en `api/brain-chat.mjs` (handler autocontenido de investigación de YouTube — no toca health/memory/lineage). Pushear `main` pelado los habría revertido en vivo. Decisión del CEO: **merge fast-forward de `codex/growth-command` a `main`** (main era ancestro, cero conflictos) y encima el commit de accesos → prod conserva lo que ya funcionaba y suma la pantalla nueva.
- ✅ **Deploy `dpl_7YGBSdZWQKAUZv3H3ireZKr2GNV3`** (githubCommitRef `main`, `githubDeployment:1`, target production): dominio sirviendo **`8da2cbe0ebe8` · commit `e0f3ea5` · builtAt 2026-09-18**. El bundle servido es **byte a byte** igual al `dist/` local. **Production Branch = main ya estaba bien configurado**: no hubo que reasignar nada, alcanzó con que hubiera un build de main más nuevo.
- 🔢 **El cache-bust es automático, no hay número que subir**: `version.version` = sha256 del bundle minificado → cambia con cualquier cambio de contenido, y el nombre del archivo (`assets/bundle.<hash>.js`) cambia con él. `version.json` vive fuera de `assets/` para heredar `must-revalidate`.
- 🧪 **Verificado en vivo**: las 6 RPC responden por PostgREST con `42501 permission denied` para `anon` (expuestas y cerradas; ⚠ **un 404 en `/rest/v1/rpc/<fn>` con payload `{}` NO es schema cache: es que PostgREST resuelve la firma por NOMBRE de argumento** — con los args correctos aparece). Corrida real contra prod sobre `qa-investor-test@`: Pausar → Casas → Rol → Reactivar, **5 filas en `inv_audit`** con `editado_por=gerencia@rentalprofitss.com` y la cuenta devuelta a su estado original.
- 🧰 **`scripts/qa-accesos-panel.mjs`** (nuevo): QA de navegador del panel — login real, columnas nuevas, las 4 acciones por fila, revocados listados, Pausar→Reactivar de punta a punta y los dos editores. `QA_PASS=… node scripts/qa-accesos-panel.mjs`. Stubea `prompt` a `''` para que la ruta "convertir en ADMIN" quede siempre cancelada.
- ⚠ **Gotcha de build en Windows (sigue vigente)**: `node scripts/build.mjs` muere en `execFileSync(node_modules/.bin/tailwindcss)` — Node en Windows no ejecuta el launcher sin extensión. Workaround local sin tocar el repo: copiar `build.mjs` **dentro de `scripts/`** (el `ROOT` sale de `import.meta.dirname`) con `sed 's|tailwindcss|tailwindcss.cmd|'` + `shell:true`, correrlo y borrarlo.
- 📌 `.vercel/project.json` local apuntaba a **`empresa-os-admin`** (el proyecto retirado): re-linkeado a `empresa-os` (`prj_NX0kx…`), backup en `.vercel/project.json.bak-empresa-os-admin`. `.vercel/` está gitignored → es local, cada máquina tiene el suyo.

## 🎯 Estado (17 Sep 2026 — 🔐 CONTROL DE ACCESOS del portal de inversionistas) · EN VIVO

- 🕳 **HUECO CERRADO**: las 3 funciones del portal (`inv_portal_resumen`, `inv_ledger`, `inv_indicadores_data`) y **24 policies RLS** traen la rama de staff `has_area('fix-flip') and not inv_is_investor()`. Como `inv_is_investor()` exigía `estado='activo'`, un inversionista **PAUSADO o REVOCADO con un área de staff colgada del profile caía en esa rama y volvía a ver LAS 23 CASAS** (caso real: `juan.sanchez49115@gmail.com`, revocado con `allowed_areas={fix-flip}`).
- 🔑 **Fix de UNA definición, sin reescribir las 24 policies**: `inv_is_investor()` pasa a significar **"tiene fila en `inv_access`"** (cualquier estado, `active` incluido). Dos excepciones EXPLÍCITAS: `profiles.role='admin'` (admin real siempre ve todo) y la columna nueva **`inv_access.es_equipo`** (cuenta del EQUIPO que además figura como inversionista — hoy solo `info.flippingrentals@gmail.com`, decisión del CEO). ⚠ **Regla: agregar a alguien a `inv_access` ahora le saca la visibilidad de staff.** Si es del equipo, marcarlo `es_equipo` desde el panel.
- 🏠 **`property_filter` cableado** (existía sin usar): array JSON de `property_id`. Se aplica en **`inv_my_props()`** → de ahí cuelgan solos ledger, params, movimientos, documentos, proyecciones y overrides; + parche puntual en `inv_portal_resumen` / `inv_portal_resumen_de` / policies `inv_holdings_read`, `inv_dist_read`, `inv_msg_read` (leen por `investor_airtable_id` y no pasaban por `inv_my_props`). `null` o `[]` = todas sus casas.
- 🚪 **Estado `'pausado'` nuevo** (el CHECK solo permitía invitado/activo/revocado). ⚠ **Sub-bug cazado**: `inv_claim_access()` ponía `estado='activo'` en cada login → **al pausado le alcanzaba volver a entrar para despausarse**. Ahora solo reclama `invitado`/`activo`.
- 🛡 **Mutaciones solo por RPC `SECURITY DEFINER` con guard `inv_es_admin()`**: `inv_admin_set_estado` (pausar/reactivar/revocar — revocar y pausar **limpian `allowed_areas` y bajan `role` admin→viewer** del profile), `inv_admin_set_property_filter`, `inv_admin_set_rol` (investor/admin, admin exige `p_confirmo`), `inv_admin_set_es_equipo`. Lectura del panel: `inv_admin_accesos()` (staff o admin) — trae rol/áreas del profile y **las casas que realmente ve cada uno**, incluidos los revocados que la consulta directa del panel no veía (`active=false`). **Nunca borrado físico**: revocar = `estado='revocado'` + `active=false`. Todo queda en `inv_audit` (trigger de `inv_access` + inserts explícitos para los cambios de `profiles`).
- 🖥 **Front**: `app.js` rutea por **`inv_contexto_acceso()`** (RPC), NO por `allowed_areas` — inversionista sin rol admin → `location.replace('/inversionista')`. Tab **Accesos** de `/inversionistas` rehecho: estado, rol/permiso, **"Casas con acceso"** con la lista real, badge **⚠️ ve de más** (áreas de staff o rol admin) y 4 acciones por fila (Pausar/Reactivar · Revocar · 🏠 Casas · 🔑 Rol).
- 🧪 **Verificado server-side con `set local role authenticated` + claims por usuario (12/12)**: revocado **con** área fix-flip → `my_props=0 · portal=0 · holdings_RLS=0` · activo normal → sus holdings exactos · `info.flippingrentals` (es_equipo) → 28 holdings / 23 casas · admin real → 28 / 23 · filtro de 1 casa → `1/1/1` · no-admin mutando → `42501` · rol admin sin confirmar → bloqueado · pausado/revocado haciendo login → `claim=0`, estado intacto. Todas las pruebas corrieron dentro de transacciones revertidas (`raise exception`).
- 🗄 Migración `20260917100000_control_accesos_inversionistas.sql` · rollback en `supabase/rollbacks/`. ⚠ `inv_props_de(text)` queda **sin EXECUTE para anon/authenticated** (toma un investor id arbitrario; solo la llaman funciones definer).
- 🙋 **Queda declarado, NO tocado**: `mekhousingllc@gmail.com` está `revocado` con `active=true` (inconsistencia legacy, sin `user_id` → inofensiva). Se normaliza sola si se la revoca desde el panel.

## 🎯 Estado (04 Sep 2026 — 🚪 la "Renta cobrada" dice DE QUÉ UNIDAD es el pago) · EN VIVO

- 🚪 **`Renta cobrada · 2026-07 · Julio 2026 — Habitación 4`**: las filas de renta del flujo por casa declaran la unidad. Sale de **`pm_units.name`** vía **`pm_payments.unit_id`** (que ya espeja la columna "unidades" de la tabla "pagos" de Airtable). Se usa `name`, **NUNCA `code`**: en las unidades legacy el code es un slug feo (`6504-STONLEIGH-HABITACI-N-1`).
- 🗄 **Migración `20260904100000_ledger_renta_por_unidad.sql`** (CREATE OR REPLACE, **misma firma RETURNS TABLE** → ningún consumidor se rompe: portal, Ledger del admin, `inv_dist_auto`, `inv_portal_resumen`). Rollback listo en `supabase/rollbacks/20260904100000_ledger_renta_por_unidad_rollback.sql`. **Se tocó UNA sola rama del UNION** (la de rentas), 3 líneas: el `left join pm_units u on u.id = pay.unit_id` y el sufijo `coalesce(' — ' || nullif(u.name,''), '')`.
- 🧮 **Cero plata movida**: el monto sigue siendo `pay.amount` tal cual — la unidad es SOLO etiqueta (cada pago de `pm_payments` ya es de una unidad). Foto antes/después idéntica al centavo en las 23 casas: **1,045 movimientos · ingresos $6,855,589.38 · gastos $8,575,408.34 · 275 rentas por $385,573.82**. El JOIN es por **primary key** → a lo sumo 1 fila, **no duplica** (275 filas de renta = 275 pagos elegibles). Además: quitarle el sufijo a cada concepto reproduce **exactamente** el texto viejo (0 diferencias).
- ⚠ **El LEFT JOIN NO filtra por `u.active`** (decisión, documentada en la migración): 40 de los 235 pagos con unidad apuntan a unidades **archivadas** (las 14 legacy con `external_id` viejo) y su `name` es bueno — filtrarlas dejaba sin etiqueta al Estudio 1 de Bramble y a las Habitaciones 2 y 5 de Childress. Acá no se LISTAN unidades de la casa (ahí sí manda "las inactivas legacy no se muestran", para no inventar unidades fantasma): se etiqueta un pago con la unidad a la que **ya apunta**. Leer el dato, no inventarlo.
- 🙋 **81 de 275 rentas no tienen unidad cargada en Airtable** → salen sin etiqueta (no rompen). Es dato faltante, no software: Barkbridge 33 · Dove Springs 16 · Childress 11 · Capitol 9 · Stonleigh 9 + 3 sueltas. Detalle y cómo completarlo en `PENDIENTE_HUMANO.md`.
- 🧪 **QA en vivo 28/28 · 0 pageerrors** (`scripts/qa-ledger-unidad.mjs`, admin + portal): Bramble multiunidad muestra Estudio 1 / Habitación 1-3 / Apartamento 1 con **saldo $9,385 y 47 movimientos intactos**; Barkbridge muestra "— Casa Completa" y sus 33 rentas sin unidad quedan igual que antes, **saldo $-3,944 · 126 movimientos**. Regresiones: acordeón **32/32**, portal **89/89**.
- ⚠ **Ojo al leer sumas históricas**: entre el 03 y el 04-sep el sync trajo la renta de **septiembre de Barkbridge corregida de $2,000 → $2,600** (y su PM 4% de $80 → $104) — el saldo pasó de −$4,520 a −$3,944. Es dato de Airtable, no efecto de este cambio (la foto antes/después del mismo día dio idéntica).
- 📌 **No hubo cambio de front**: `inv_ledger` es server-side, así que las dos pantallas mostraron la unidad apenas se aplicó la migración, sin deploy.

## 🎯 Estado (03 Sep 2026 — 📅 "Todos los movimientos" AGRUPADO POR MES en las dos vistas) · EN VIVO

- 📅 **La lista de movimientos del flujo de caja pasa a acordeón por mes** (mes+año, más reciente arriba y ABIERTO; el resto cerrados) + botones **Expandir/Colapsar todo**. El encabezado del mes muestra el resumen **aunque esté cerrado**: `Ingresos $X · Gastos −$Y · Neto $Z` (verde si ≥0, rojo si <0). Dentro del mes: **primero los ingresos, debajo los gastos**, cada subgrupo por fecha descendente.
- 🔁 **UNA sola función de render para el Ledger del admin y el Flujo Mensual del portal**: `invEngine.movsGrupos / movsAbierto / movsSetTodos / movsTabla` en **`os/inv-engine.js`** — el único archivo que cargan el bundle del admin **y** el portal standalone. Cada vista solo aporta su escape, su formato de plata, sus `<th>` y el `<tr>` de cada fila. **Tocar el agrupado toca las dos pantallas a la vez** (ya no se pueden desincronizar).
- 🧮 **Cero cambios de cálculo o de fuente**: el **saldo se sigue calculando UNA vez en orden cronológico real ANTES de agrupar** y viaja con la fila — reordenar no lo recalcula. Los totales del encabezado suman los montos que YA existen con la misma regla `pnlSi`; lo que es **capital (draw, cash-out, aporte, compra, distribución) se declara aparte** como `· capital ±$X (P&L NO)` y **nunca entra al neto**, para no contradecir el "Detalle año → mes" ni el Saldo de caja del período. Cuadre elegante: el **Neto del mes = el saldo de ese período** que ya daba el filtro (Barkbridge jun-26 −$294 en las dos pantallas).
- ⚠ **Consecuencia del orden pedido**: con ingresos arriba y gastos abajo, la columna de saldo **ya no se lee como una corrida hacia abajo** (cada fila muestra su saldo real, que es de otro instante). Es inherente al reordenamiento; el ⓘ de la columna lo dice.
- ⚠ **Divergencia PRE-EXISTENTE visible ahora en los encabezados**: el admin agrupa por **fecha** y el portal por **mes contable** → jul-26 Barkbridge da `−$3,934 / 9 movs` en el admin y `−$3,869 / 8 movs` en el portal (un Spectrum del 12-jul tiene mes de renta agosto). **No se tocó** (mover eso cambiaría números fuera del pedido).
- 🧪 **QA en vivo en el dominio oficial 32/32 · 0 pageerrors** (`scripts/qa-movs-agrupados.mjs`, admin + portal en la misma sesión): Julio 2026 Barkbridge `Ingresos $3,500 · Gastos −$7,434 · Neto −$3,934` (admin) y `−$7,369 / −$3,869` (portal), idénticos a la suma manual de `inv_ledger` en Supabase; cadena de saldo validada día por día sobre las 126 filas. **Regresión del portal 89/89**.
- ⚠ **Gotcha para futuros QA**: los scripts que recorren `table tbody tr` ahora ven **filas de encabezado de mes (1 celda con `colspan`)** y **no ven las filas de los meses cerrados** → hay que llamar `ipMesTodos(true)` / `iaMesTodos(true)` antes de leer, y **el saldo anterior ya no es "la fila de abajo"**: ordenar por fecha. `scripts/qa-flujo-portafolio-deuda.mjs` quedó adaptado a las dos cosas (vuelve a dar 89/89).

## 🎯 Estado (27 Ago 2026 · 6 — 🔢 Property Management al 4% + ⓘ en Subtotales) · EN VIVO

- 🔢 **PM de 5% → 4%** (migr `20260827170000`): **un `update` de una fila, sin backfill**. Como el ítem se CALCULA en `inv_ledger` leyendo `ff_uw_config.inv_pm_fee_pct`, los meses no editados se recalcularon solos, la etiqueta pasó a `Pago Property Management (4%)` y **los editados a mano no se tocaron** (usan el override, no el %). Barkbridge 2,700→108 · 3,500→140 · 2,000→80 · 1,300→52 · Michelle 3,700→148 · Virginia 5,100→204 y 4,490→179.60.
- ✅ **Comprobación cruzada elegante**: el saldo histórico de Barkbridge subió de $4,085.81 a $4,393.24 → **+$307.43 = exactamente el 1% de sus $30,743.47 de renta cobrada**. El punto porcentual que se bajó, ni un centavo más.
- ⚠ **`inv_pm_fee_pct` (Ledger) y `pm_fee_pct` (underwriting) AHORA VALEN LOS DOS 4** — son claves distintas con significados distintos. **NO consolidarlas**: mover una movería los números de la otra pantalla.
- ⓘ **"¿de dónde sale?" en Subtotales por categoría** (Ledger del admin), en dos niveles: botón en el encabezado que despliega el desglose completo (qué suma cada categoría, de qué tabla sale, y qué significa **"P&L NO"** = capital/financiamiento, no entra al P&L ni mueve el saldo) + un ⓘ por fila al pasar el mouse. Diccionario `IA_SUBT_TIP` **con fallback**, así una categoría nueva nunca queda sin explicar.
- 🐛 **Efecto secundario corregido en el portal**: el Property Management caía en **"otros"** en el desglose de Gastos operativos (su concepto no empieza con "Gasto", el patrón que trae Rentas). Ahora se nombra **"Property Management"** con su ⓘ; también se agregó ⓘ por línea en la itemización del servicio de deuda.
- 🧪 **QA en vivo**: filtro + ⓘ **47/47**, portal **89/89**, 0 pageerrors. Bundle `85b87596aaad`, commit `09dafbe`.

## 🎯 Estado (27 Ago 2026 · 5 — 📆 el filtro del Ledger gana nivel de AÑO) · EN VIVO

- 📆 **Un solo `<select>` con tres niveles** (mismo look, sin controles nuevos): `Todos los años` · `<optgroup>` por año con `Todo 2026` + sus meses. Años y meses se arman **dinámicamente de las fechas reales de esa casa** — si mañana hay 2027 aparece solo y **no se listan períodos vacíos**.
- 🔑 **El valor de cada opción es el PREFIJO de la fecha** (`"2026"` / `"2026-06"`) → el filtro sigue siendo el mismo `startsWith` de antes: **cero cambios de cálculo o categorías**, solo el rango que se muestra y se suma.
- ♻️ **El recálculo salió gratis**: Saldo de caja, Servicio de deuda, Subtotales por categoría, Fuentes y tabla ya derivaban todos de `vis` → cambian los cinco con el período. Los dos totales del encabezado ahora dicen cuál es (`Saldo de caja (P&L) ⓘ · año 2025: $2,568`).
- 🧪 **QA en vivo 41/41 · 0 pageerrors** (`scripts/qa-ledger-filtro-anio.mjs`, Barkbridge): todos $4,085.81 / 117 movs · 2025 $2,568.04 / 59 · 2026 $1,517.77 / 58 · jun-26 −$314.42 / 9. **Los años suman exacto el total** (2,568.04+1,517.77=4,085.81 · 59+58=117): el filtro parte, no pierde ni duplica. Regresión del portal: 89/89.
- ⚠ **Divergencia PRE-EXISTENTE anotada**: el Ledger del **admin** agrupa por `fecha` y el Flujo Mensual del **portal** por `mes` (billing_ym). Una renta de junio cobrada en julio cae en meses distintos según la pantalla. No se tocó (movería números fuera del pedido).

## 🎯 Estado (27 Ago 2026 · 4 — 🧾 ítem automático "Pago Property Management (5%)") · EN VIVO

- 🧾 **Ítem nuevo por casa y por mes** (migr `20260827160000`): **5% del total de renta cobrada del mes** (fuente `pm_payments`), categoría **operativo** (resta del mes, del neto y del saldo), fecha = **último día real del mes**, concepto `Pago Property Management (5%)`, `subcategoria='pm_fee'`, fuente `OS:pm_fee(...)`.
- 🏗 **NO se materializa: se CALCULA dentro de `inv_ledger`** sobre `pm_payments` → se recalcula solo cuando cambia la renta, sin filas que envejezcan. Lo único persistido son las **ediciones manuales** en **`inv_pm_fee_overrides`** (una por casa+mes, audit trigger, RLS fix-flip, unique parcial sobre activas) — mismo patrón que `inv_param_overrides`.
- ✏️ **Es el ÚNICO auto EDITABLE**: con override activo manda su monto/fecha, el concepto pasa a `(editado a mano)` y el recálculo **no lo pisa**; los no editados sí se recalculan. `eliminado=true` lo quita del mes; **↩ volver al automático** = `active=false` (queda la historia). Botones ✎/🗑/↩ en la fila de "Auto-importados".
- 🚫 **Anti-duplicado**: si ya hay un property management cargado a mano para esa casa+mes en **`pm_expenses` O en `inv_cashflow_real`**, el automático NO se genera (el manual manda). Caso real: Michelle jul-26 tiene $148 manual → no se generaron los $185.
- ⚠ **GOTCHA CARO (lo cazó la verificación a mano)**: `ff_uw_config.pm_fee_pct` **ya existía con value=4** y es el PM fee del **UNDERWRITING**; además esa tabla guarda porcentajes **ENTEROS** (`lender_fee_pct=2`, `wholesale_fee_pct=3`). Reusarla dio `"Pago Property Management (400%)"` y montos 4× la renta. **Clave propia `inv_pm_fee_pct`=5** + normalización defensiva (`>1 → /100`), sin tocar la del underwriting. **Regla: antes de sembrar una key en `ff_uw_config`, chequeá que no exista y con qué escala.**
- 🧪 **QA en vivo 89/89 · 0 pageerrors**: Barkbridge 2,700→135 · 3,500→175 · 2,000→100 · 1,300→65 · Michelle 3,700→185 · Virginia 5,100→255 y 4,490→224.50, todos a fin de mes; Virginia may-26 sin renta → sin ítem; edición a mano probada (120/28-jun persiste, otros meses siguen auto) y revertida con ↩.
- 📋 Antes/después mes a mes y las 4 reglas con su caso probado en `AUDITORIA-RENTAL-PROFITSS.md` (pasada 27-ago · 4).

## 🎯 Estado (27 Ago 2026 · 3 — 💸 el SERVICIO DE DEUDA resta como GASTO del mes) · EN VIVO

- ⚠ **CONTRATO NUEVO del Ledger**: el **pago mensual recurrente de deuda** (`Pago interés HML` / `Pago Refi 30 años`, de `ff_hml_payments`) pasa de `categoria='financiero'` a **`categoria='operativo'`** conservando `subcategoria='servicio_deuda'`. Como `invEngine.pnlSi()` ya trata `operativo` como P&L SÍ, **ahora RESTA del mes y BAJA el saldo del Ledger**, igual que un utility (decisión CEO 27-ago). Concepto y trazabilidad (`FF:ff_hml_payments`) intactos.
- 🚫 **NO se movió el resto de `financiero`**: desembolso HML y draws, cash-out del refi, aporte de capital, **fee HML puntual (`subcategoria='fee_hml'`)** y distribuciones siguen P&L NO — son capital, no gasto del mes. Verificado: 12 conceptos de capital siguen sin mover el saldo.
- 🔴 **REGLA DURA ANTI DOBLE CONTEO**: todo consumidor que sume "gastos operativos" y "servicio de deuda" POR SEPARADO **debe excluir `subcategoria='servicio_deuda'` del bucket operativo**, o lo resta dos veces. Los 4 que existen ya lo hacen (migr `20260827150000` + front): `inv_dist_auto` · `inv_portal_resumen` · `inv_portal_resumen_de` · `os/inv-portal.js` (`renderFlujo` + `ipMetrics`). **Los netos NO cambiaron** (Barkbridge jun-26 sigue −149.08 = renta 2,000 − oper 569.35 [5 movs] − deuda 1,579.73 [1 mov]); lo que cambió es el SALDO.
- 📊 **Antes → después del saldo acumulado**: Barkbridge 26,455.36 → **5,622.98** · Michelle 30,052.00 → **2,826.18** · **Bartlett 8,250.00 → −14,472.10** (cobró 8,250 de renta y pagó 22,722.10 de interés HML — antes el Ledger decía +8,250).
- 📖 **Guía de clasificación actualizada** (`IA_CAT_GUIA`/`IA_PNL_TIP`/`iaSugerirCat` en `os/inv-admin.js`): **Operativo (RESTA del mes)** ahora incluye el pago mensual de deuda; **Financiero (NO resta)** = draw, aporte, cash-out, distribuciones. La sugerencia automática evalúa la regla de deuda ANTES que la de capital (11/11 casos correctos: "Pago interés HML"→operativo, "Draw 3"→financiero).
- 🏷 **La columna del Ledger pasa de "Saldo operativo" a "Saldo de caja" ⓘ** (portal + admin): ahora incluye la deuda y decir "operativo" sería mentir. El **NOI se sigue mostrando aparte y SIN deuda** — es lo que lo hace comparable entre casas.
- 🧪 **QA en vivo 65/65 · 0 pageerrors**: en las 3 casas el pago de deuda baja el saldo (Barkbridge $1,291 → −$288) y ya no lleva "P&L NO"; `Draw 4` / `Distribución` / `Desembolso Hard Money` dejan el saldo **igual**. Bundle `bca280b2dd4e`, commit `713d7bb`.
- ⚠ **Alcance**: es el Ledger del inversionista. **NO** toca `v_pnl_casa`, `inv-engine`, indicadores ni vistas contables.

## 🎯 Estado (27 Ago 2026 · 2 — 🏦 HML vs REFI 30: monto, fecha y concepto REALES de Airtable) · EN VIVO

- 🐛 **Barkbridge jun-26 decía "Pago interés HML −$1,600" cuando el real es "Pago Refi 30 años −$1,579.73".** El `$1,600` **NO era un valor modelado ni hardcodeado**: era `ff_hml_payments.pago_hml`, el espejo de la columna "Pago HML" **con el valor que tenía antes de la edición del CEO**. (Descartado: `ff_deals.hml_payment`=0 y `ref30_payment`=1,579.73 ya estaban bien.)
- ✅ **El sync NO era el bug**: `sync-ff-airtable` ya mapea los 6 campos de `tblV4wNA8hNs5Mmk8` (HML `fld7y5uQLeJlCHgno`/`fldDe1BDW4fP5s3WR` · **Refi `fldWACGPEKKhLp206`/`fldlDpPWUnYhIsETm`** · fee · check). Prueba: ya había 18 filas con `ref30>0` antes de tocar nada.
- 🔴 **CAUSA RAÍZ: NO existía cron para `sync-ff-airtable`** (Rentas :00/:15/:30/:45 · Remodelación :30 · workers :05 · ClickUp :00 — FF ninguno). El espejo estaba del 26-ago 23:54 y Airtable se editó el 27-ago 16:00 → **el CEO editaba y no lo veía**. Creado **`sync-ff-airtable-hourly`** (jobid 119, `20 * * * *`). Quitar con `select cron.unschedule('sync-ff-airtable-hourly')`.
- 📏 **REGLA POR MES (ya estaba bien, se conserva)**: `Pago HML > 0` → "Pago interés HML" con "Fecha de Pago - HML" · `Pago Ref 30 > 0` → **"Pago Refi 30 años"** con "Fecha de pago Ref 30". Ambos `subcategoria='servicio_deuda'`, `financiero` → P&L NO. **El monto SIEMPRE de `ff_hml_payments`.** Migr `20260827140000`: renombra el concepto al texto del CEO + **mata el descarte silencioso** (un pago con monto>0 sin *su* fecha se caía del ledger; ahora cae a la otra fecha del mismo registro — hoy 0 filas lo usan, es un seguro).
- 🧪 **QA en vivo 53/53 · 0 pageerrors**: Barkbridge jun-26 `$2,000 · −$569 · −$1,580 · −$149` con concepto "Pago Refi 30 años" (y "Pago interés HML" en los meses 2025, cero "1,600") · Michelle muestra la transición HML→Refi (jun 2,116.13 → jul/ago 3,032.26) · **311 Bartlett (NO refinanciada): sus 14 movimientos todos "Pago interés HML", 0 con "Refi 30"**.
- ⚠ **Hallazgos abiertos**: (1) **1100 Echo lane 2026-06-02 tiene los DOS** pagos en la misma fila (HML 2,770 + Refi 3,084.34) — única en 166, mes de transición; el ledger muestra ambos. (2) **`v_pnl_casa.interes_hml_real` suma solo `pago_hml`, ignora `ref30`** → `utilidad_neta_post_interes` queda optimista en casas refinanciadas. Los dos, decisión del CEO.
- 📋 Detalle mes a mes y antes/después en `AUDITORIA-RENTAL-PROFITSS.md` (pasada 27-ago · 2).

## 🎯 Estado (27 Ago 2026 — 🏦 el SERVICIO DE DEUDA también baja el flujo del PORTAFOLIO) · EN VIVO en `empresa-os.vercel.app`

- ✅ **La pestaña Flujo Mensual ya estaba bien y ya estaba en vivo** en el dominio oficial (llegó a `main` con el merge de consolidación `5b92bf2`): verificado byte a byte que `empresa-os.vercel.app/os/inv-portal.js` == `main` HEAD.
- 🐛 **Lo que faltaba era el PORTAFOLIO**: la tarjeta "Flujo del último mes" de *Tus casas de un vistazo* se calculaba **aparte**, en el CTE `ultm` de `inv_portal_resumen`: `pm_payments − pm_expenses`. No leía `ff_hml_payments` y **sí** restaba la "Hipoteca" espejada en `pm_expenses` (4916 Barkbridge: espejo $1,579.73 vs deuda real $1,600.00 — el mismo concepto con dos números en la misma pantalla). Migr `20260827120000`: `ultm` ahora lee **una sola fuente**, `inv_ledger(pid)` → `renta − operativos − servicio_deuda`, por **mes contable** y cortado a HOY.
- ⚠ **`inv_portal_resumen_de()` es una COPIA de `inv_portal_resumen()`** (la usa "Ver como inversionista" del admin vía `inv_portal_como`) y arrastraba el mismo `ultm` viejo — lo cazó el QA en carga real (el inversionista real ya veía bien, el admin no). Migr `20260827130000`. **Regla: tocar una de las dos obliga a tocar la otra hasta que se unifiquen.**
- 🔴 **Hallazgo abierto (declarado, NO tocado)**: esa copia además quedó con el **déficit viejo** (`greatest(0,−v_pnl_casa.utilidad_neta_post_interes)`) mientras la principal usa `ff_deals.deficit_total` desde el 20-ago → el admin puede ver un déficit distinto al del inversionista. Decisión del CEO.
- 🧪 **QA en carga real 34/34 · 0 pageerrors** (`scripts/qa-flujo-portafolio-deuda.mjs`, login por formulario en el dominio oficial): Barkbridge jun-26 `$2,000 · −$569 · −$1,600 · −$169` en pantalla, idéntico al neto de `inv_dist_auto`; total del portafolio (4 casas) `−$6,023` = suma de las tarjetas.
- 🔴 **Dato faltante, no código**: varias casas (Barkbridge, Nesting…) **no tienen cargado el pago de deuda de jul/ago** en `ff_hml_payments` → esos meses muestran flujo sin deuda. Cargar en Airtable.
- 📋 **Antes/después casa por casa (23) + cómo revertir + cómo re-verificar: `AUDITORIA-RENTAL-PROFITSS.md`.**
- ⚠ Gotcha de build: en Windows `npm run build` falla al invocar `node_modules/.bin/tailwindcss` (Node no ejecuta `.cmd` sin `shell`). En Vercel (Linux) corre bien; **`scripts/build.mjs` no se tocó**.
...
- **Estándar permanente de excelencia:** una tarea no se considera terminada solo porque compila o responde. Debe quedar validada funcionalmente, consistente con las fuentes de verdad, comprensible para una persona no técnica y visualmente mejor en toda superficie tocada. Cada cambio de UI incluye estados de carga/vacío/error/éxito, escritorio+móvil, teclado, contraste, temas y una comprobación visual final. No se agrega decoración sin propósito ni se inventa actividad; el diseño debe aumentar claridad, confianza y velocidad operativa.
...
- Auditoría operativa local posterior (26 Ago 2026, pendiente de aprobación/deploy): Centro de mando agrega una matriz de preparación de seis pruebas por agente (ejecutor, horario, responsable, misión/tareas, evidencia reciente y estado promovido) y reincorpora los controles vivos de ocupación/linaje. `npm run audit:jarvis` valida estructuralmente las 23 automatizaciones contra implementación, autenticación, bitácora y horario versionado.
- El Auditor de Integridad Financiera y Datos emite además `triage_excepciones_financieras`: prioriza los hallazgos vigentes por severidad, impacto, antigüedad y responsable funcional. Es estrictamente observacional; nunca modifica QuickBooks, pagos, cobros, préstamos, nómina ni marca hallazgos como resueltos sin soporte confirmado en la fuente de verdad.
- Auditoría local 28 Ago 2026, pendiente de migración/deploy: `run_financial_source_scan()` convierte C10/C11/C12/C15 en controles autónomos diarios (06:20 Austin), refresca desde los espejos canónicos y solo resuelve cuando la regla deja de disparar. C11 compara el cobro de Remodelación contra **fondos disponibles para obra** (`draw − intereses − servicios − muebles − extensiones`), no contra el draw bruto; C15 reconoce fecha/monto de refi o venta como evidencia de salida del HML aunque el stage siga `rentada`. La migración completa pasó ejecución transaccional en producción con `ROLLBACK`; no dejó función, cron ni datos persistidos. Test Node 24: 9 aserciones; Jarvis: 26/26 automatizaciones y 12/12 controles; build verde.
- Continuidad local 28 Ago 2026, pendiente de deploy: Jarvis muestra la salud de cada integración con evidencia y recuperación concreta; no presenta un cron disparado como sincronización exitosa. Las rutas `clickup-writeback` y `clickup-execute` exigen administrador y bloquean toda escritura si `clickup_sync_log` tiene error o más de dos horas. El auditor estructural prueba ambas condiciones (2/2).
- Verdad de automatizaciones 28 Ago 2026, pendiente de migración/deploy: `v_automation_effective_health` combina la señal del cron con el resultado real de ClickUp, sincronización de Rentas, Remodelación y paridad de las cuatro entidades de Fix & Flip. Los 55 horarios versionados tienen dueño, criticidad y máximo silencio; `v_automation_expectation_gaps` escala automáticamente cualquier cron futuro sin catalogar. `run_automation_watchdog()` conserva causa/fuente/fecha de evidencia y nunca marca saludable una integración cuyo reloj corrió pero cuyo trabajo falló. Jarvis muestra el control “Trabajo automático” y abre el diagnóstico cuando exista un resultado sin confirmar.
- Seguridad de canales local 28 Ago 2026, pendiente de deploy: `whatsapp-send` y `whatsapp-send-cloud` dejan de aceptar llamadas anónimas; el emisor genérico restringe los tipos de mensaje y ambos usan el contrato común `service_role` interno o administrador autenticado. El cambio conserva los crons internos y bloquea emisores externos no autorizados.
- Webhook WhatsApp local 28 Ago 2026, pendiente de secret/deploy: los POST de Meta deben verificar `x-hub-signature-256` con `META_APP_SECRET` antes de registrar mensajes o cerrar tareas. El secret todavía no existe en el proyecto remoto; no desplegar esta función hasta configurarlo en Meta/Supabase y probar un webhook firmado real.
- Carry-over operativo local 28 Ago 2026, pendiente de deploy: `pm-daily-close` ya no confunde comentar con reprogramar. Usa la acción ClickUp `reschedule`, cambia `due_date`, cuenta únicamente respuestas confirmadas y deja una alerta persistente cuando alguna tarea no se movió; el mensaje al equipo declara éxitos y fallos reales.
- OAuth QuickBooks local 28 Ago 2026, pendiente de deploy: iniciar una conexión pasa a `POST /authorize` con administrador autenticado; el callback solo acepta un `state` HMAC ligado a empresa, nonce y vencimiento de diez minutos. `/status` también queda administrativo. Esto evita vincular un realm externo a una empresa usando un `state` manipulable.
- Cobertura humana local 28 Ago 2026, pendiente de deploy: el panel permite que un administrador confirme titular y respaldo distintos para los nueve roles críticos versionados usando candidatos con permisos compatibles, incluido el Auditor de Integridad Financiera y Datos agregado por su migración específica. No asigna por inferencia: experiencia, disponibilidad y aceptación requieren una atestación explícita. `assign_operational_role()` valida administrador, actividad y accesos; luego actualiza, conserva historial inmutable y ejecuta la revisión integral en una sola transacción.
- Resolución financiera local 29 Ago 2026, pendiente de deploy: `v_financial_findings_actionable` traduce todos los controles financieros actualmente abiertos a frente, responsable, evidencia obligatoria, siguiente acción y prioridad operativa. Jarvis usa esta cola viva en el informe financiero; un código técnico o un total agregado ya no reemplaza las instrucciones de resolución.
- Cobertura empresarial local 29 Ago 2026, pendiente de deploy: `v_business_agent_coverage` cruza cada empresa activa de `pm_companies` con los agentes no legacy y exige seis capacidades mínimas: dirección, ejecución, finanzas, optimización, reportes e integridad. La migración pasó contra las cuatro empresas reales con `ROLLBACK`: 4/4 cubiertas; no dejó cambios persistidos.
- El Director de Continuidad mantiene un vigilante horario de automatizaciones críticas. La salud exige una señal dentro de la frecuencia declarada; un cron ausente, desactivado, fallido, vencido o que nunca arrancó se escala aunque no exista un error explícito. El vigilante no reintenta acciones sensibles ni altera datos.
- La certificación de ausencia prolongada usa once compuertas. Además de agentes, cobertura humana, decisiones, ocupación, linaje, QuickBooks, reportes y fallos de cron, exige señal vigente de automatizaciones, cero hallazgos financieros críticos y cero trabajos aprobados sin ejecución confirmada. Mientras una falle, el estado canónico es `no_listo`.
- Auditoría de seguridad 27 Ago 2026: cero tablas públicas sin RLS y cero vistas públicas sin `security_invoker`. Las cinco vistas públicas de mercado conservan lectura anónima mediante permisos explícitos y limitados sobre sus fuentes de referencia; `v_ff_stage_duration` respeta el RLS de su tabla base.
- La salud diaria de integraciones exige evidencia operativa de Airtable, ClickUp, QuickBooks y RentCast; una llave guardada no basta. RentCast usa caché de 30 días y telemetría mensual atómica de éxitos/fallos/último estado. No se bloquea por una cuota inventada: `monthly_limit` permanece configurable según el plan real.
- La Reunión Matutina consume, además de las fotos de Rentas/Remodelación/Fix & Flip, la certificación de ausencia, el triage financiero, el SLA de decisiones y la salud de integraciones. Una prueba manual posterior al deploy produjo una directiva con fuentes y escaló 7/10 compuertas, 43 alertas financieras críticas y 121 decisiones vencidas; el agente sigue siendo de solo lectura.
- Aprobación y ejecución quedan separadas de forma verificable. `run_approved_work_review()` vigila las propuestas aprobadas sin `executed_at`, excluye los registros históricos de creación de agentes, asigna responsable y próxima acción por tipo y alimenta la Reunión Matutina. La UI dice “aprobada · pendiente del responsable”; solo `executed_at` permite mostrar ejecución confirmada. El control nunca envía cobros, ajusta contabilidad ni modifica sistemas fuente.
- La Reunión Matutina mantiene un único informe y una única memoria por día, pero los actualiza cuando vuelve a correr. El dedup evita duplicados sin congelar una foto operativa vieja durante el resto del día.
- El rol `agentes_ia_exec` solo puede refrescar `payload` y `updated_at` de informes cuyo origen sea `ejecutor`; no recibe permiso para cambiar tipo, corte, autor, estado, archivo ni informes humanos.
- Cobertura humana muestra candidatos derivados exclusivamente de perfiles activos y acceso compatible, excluyendo cuentas QA/test. Son una ayuda para decidir, no una asignación: competencia, disponibilidad, aceptación del rol y respaldo requieren confirmación administrativa explícita.
- Jarvis reconecta el ejecutor autenticado `clickup-writeback` para `archivar_tarea`, `refechar_tarea` y `reasignar_tarea`: después de la revisión y confirmación humana, la acción se aplica en ClickUp y solo entonces queda `ejecutada` con `executed_at`. Los demás tipos continúan como aprobación pendiente de ejecución; nunca se enrutan pagos, cobros, mensajes ni contabilidad por este camino.
- `clickup-writeback` exige rol administrativo en el servidor, no solo visibilidad del botón. Un usuario autenticado sin administración no puede ejecutar cambios de tareas invocando directamente la función.
- `jvLoad` dejó de ejecutar una consulta por agente: una sola lectura consolidada de `agent_audit_log` alimenta última corrida/evidencia de todo el equipo. Esto elimina 23 viajes redundantes a Supabase por apertura de Jarvis.
- “Negocio que funciona por sí mismo” significa automatizar observación, análisis, priorización, reportes y propuestas con evidencia; pagos, contratos, mensajes externos, cambios contables y decisiones sensibles mantienen aprobación humana explícita.
- Continuidad operativa (27 Ago 2026): `Director de Continuidad Operativa` revisa diariamente evidencia de agentes, fallos, decisiones envejecidas, crons, fotos ejecutivas por empresa y frescura de QuickBooks; deja informes diarios/semanales/mensuales y eleva excepciones a la reunión del Cerebro. No ejecuta acciones sensibles.
...
- El mapa principal deja atrás las cuatro tarjetas grandes: adopta un **organigrama compacto de una sola vista**, inspirado en la referencia visual aprobada por Nicolás. Dibuja Cerebro Ejecutivo → responsables por área → especialistas con conexiones visibles, estado derivado de evidencia y una ficha operativa fija a la derecha. Zoom y “Acomodar” son controles reales; todos los nodos usan `agent_registry`, nunca nombres o actividad de demostración.
- Optimización Fix & Flip ya no queda inútil mientras aprende: su ejecutor opera en modo **observabilidad supervisada**, revisando etapas faltantes y frescura de `ff_deals` con evidencia en `agent_audit_log`; solo crea una propuesta si hay problemas. Jarvis lo reconoce como funcionando únicamente después de una corrida `ok` cuyo output declara `operational:true`, aunque el registro conserve `dry-run` hasta habilitar estadísticas. Las medianas y alertas por duración siguen cerradas hasta alcanzar 3 intervalos reales, 2 propiedades y 2 etapas. No reconstruye historia ni convierte baselines en transiciones.
...
- Ciclo de vida de alertas de servicios (29 Ago 2026): `rentas-financiero?mode=servicios` reconcilia el conjunto completo `svc:` y retira suavemente hallazgos que desaparecen de la fuente, sin aprobar ni ejecutar correcciones. Prueba productiva: 11 detectados, 11 refrescados, 0 retirados porque los 11 continúan vigentes; aislamiento de credenciales y documentos sensibles en PASS.
- Frescura visual de agentes semanales (29 Ago 2026): horarios declarados con un día de semana (por ejemplo, `Miércoles 08:00`) usan ventana semanal. Antes caían erróneamente a 72 horas y podían mostrar `requiere configuración` aunque el cron y la última evidencia semanal estuvieran vigentes. El vigilante de base conserva su máximo de silencio propio; no se relajó la prueba operativa.
