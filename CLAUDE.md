...
## 🎯 Estado (29 Jul 2026 — 📊 INFORMES AUTOMÁTICOS DE RENTAS: los 3 informes manuales, vivos) · EN RAMA `feat/informes-rentas` (sin deployar — falta correr ci:gate con SB_KEY)
...
- ⏭ **PENDIENTE para deployar**: correr `SB_KEY=sb_secret_… npm run ci:gate` (verde) → merge a main → Vercel. No lo hice: no tengo la service key para confirmar el gate (regla: deploy solo tras gate verde). QA en browser sobre prod tras deploy (generar los 3, ver PDF, 0 errores consola). Programación de envío por email/WhatsApp a la bandeja: el motor `_pm-send.mjs` (Resend + WhatsApp + Storage `pm-reports`) ya existe — cablear cuando se pida (hoy el cron deja el borrador listo, humano emite).
