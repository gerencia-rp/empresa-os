...
function apProvisionalBanner(rec) {
  ...
  return '<div class="ap-card" style="padding:13px 16px;margin-bottom:12px;border-color:rgba(231,182,94,.5);background:rgba(231,182,94,.07)">'
    + '<b style="color:var(--amber,#dca94f)">' + osIcon('alert') + ' ARV PROVISIONAL — confirmá los datos del subject</b>'
...
function apVistaSenales(rec, inp) {
  ...
  const filas = tri.senales.map(x => {
    ...
    return '<div class="ap-mrow"><span>' + AP_E(x.nombre) + ' <span class="ap-plain" style="font-size:10.5px">' + AP_E(x.sub || '') + '</span></span><b>' + AP_M(x.valor)
      + (x.nombre !== 'Comps (motor)' && dPct != null ? ' <span style="font-size:10.5px;color:' + (Math.abs(dPct) > (tri.umbral || 8) ? 'var(--amber,#dca94f)' : 'var(--mut,#a69d8c)') + '">' + (dPct > 0 ? '+' : '') + dPct.toFixed(1) + '%</span>' : '') + '</b></div>';
  }).join('');
  return '<div class="ap-card" style="padding:16px 18px;margin-bottom:14px"><div class="ap-lab">' + osIcon('compass') + ' Triangulación — señales independientes' + (tri.warn ? ' · <span style="color:var(--amber,#dca94f)">' + osIcon('alert') + ' divergen ' + tri.divergenciaPct.toFixed(0) + '%</span>' : ' · <span style="color:var(--pos,#63c08e)">✓ consistentes</span>') + '</div>'
    + filas
...
function apArvSlide(val) {   // oninput (drag): update EN VIVO sin re-render → fluido
  ...
  set('ap-arv-tag', st.arvManual ? '<span style="color:var(--amber,#dca94f)">ajustado por vos</span>' : '<span style="color:var(--pos,#63c08e)">del motor</span>');
...
function apArvSlider(rec) {
  ...
  return '<div class="ap-arvsld" style="margin-top:16px">'
    ...
    + '<div><div class="ap-lab">ARV elegido · <span id="ap-arv-tag">' + (manual ? '<span style="color:var(--amber,#dca94f)">ajustado por vos</span>' : '<span style="color:var(--pos,#63c08e)">del motor</span>') + '</span></div>'
    ...
    + '<div id="ap-arv-guard" style="display:' + (guard ? 'block' : 'none') + ';margin-top:6px;font-size:11.5px;color:var(--amber,#dca94f);background:rgba(231,182,94,.1);border:1px solid rgba(231,182,94,.32);border-radius:8px;padding:6px 10px">' + guard + '</div>'
...
function apCSS() {
  ...
  st.textContent = [
    '.ap-grad{background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0))}',
    ...
    '.ap-lab{color:var(--mut,#a69d8c);font-size:11px;text-transform:uppercase;letter-spacing:.8px;font-weight:700}',
    ...
    '.ap-btn{background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));color:#fff;font-weight:700;border:none;border-radius:10px;padding:9px 16px;font-size:13px;cursor:pointer;white-space:nowrap;flex-shrink:0}',
    ...
    '.ap-stat .n{font-size:16px;font-weight:700}.ap-stat .t{color:var(--mut,#a69d8c);font-size:10.5px}',
    ...
    '.ap-big{font-size:44px;font-weight:800;background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));-webkit-background-clip:text;background-clip:text;color:transparent;line-height:1.05;margin:6px 0 2px}',
    ...
    '.ap-rline .fill{position:absolute;left:16%;right:16%;top:0;bottom:0;background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));border-radius:6px;opacity:.45}',
    ...
    '.ap-rvals{display:flex;justify-content:space-between;color:var(--mut,#a69d8c);font-size:11.5px}.ap-rvals b{color:var(--ink,#eaf0ff)}',
    '.ap-fchip{background:var(--card,rgba(255,255,255,.04));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:20px;padding:6px 13px;font-size:12px;color:var(--mut,#a69d8c);display:inline-flex;align-items:center;gap:5px}',
    ...
    '.ap-kv span{color:var(--mut,#a69d8c)}.ap-kv b{text-align:right}',
    ...
    '.ap-pin.s{background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));color:#fff}',
    ...
    '.ap-comp.hl{border-color:var(--a1,#6fbf95);box-shadow:0 0 0 2px var(--a1,#6fbf95)}',
    ...
    '.ap-wt{display:inline-block;font-size:11px;font-weight:800;color:#fff;background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));padding:2px 9px;border-radius:20px}',
    '.ap-pill{font-size:11px;padding:2px 9px;border-radius:20px;background:var(--glass,rgba(255,255,255,.06));border:1px solid var(--line,rgba(255,255,255,.12));color:var(--mut,#a69d8c)}',
    '.ap-adj{display:flex;justify-content:space-between;font-size:11px;padding:2px 0}',
    '.ap-adj span{color:var(--mut,#a69d8c)}',
    '.ap-pos{color:var(--pos,#63c08e);font-weight:700}.ap-neg{color:var(--neg,#e4756a);font-weight:700}',
    ...
    '.ap-grid th:first-child,.ap-grid td:first-child{text-align:left;position:sticky;left:0;background:var(--bg2,var(--card,#1d1913));color:var(--mut,#a69d8c);font-weight:600;font-size:11.5px;z-index:2}',
    ...
    '.ap-grid .sec td{background:var(--glass,rgba(255,255,255,.04));color:var(--mut,#a69d8c);font-size:10.5px;text-transform:uppercase;letter-spacing:.6px;font-weight:800;text-align:left}',
    '.ap-grid .best td{box-shadow:inset 0 2px 0 var(--a1,#6fbf95)}',
    '.ap-cbx{accent-color:var(--a1,#6fbf95);width:15px;height:15px;cursor:pointer}',
    ...
    '.ap-slider{-webkit-appearance:none;appearance:none;width:100%;height:8px;border-radius:6px;background:linear-gradient(90deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));outline:none;cursor:pointer;margin:0}',
    ...
    '.ap-ticklbl{display:block;white-space:nowrap;margin-top:5px;font-size:10px;color:var(--mut,#a69d8c);line-height:1.25}',
    ...
    '.ap-seg button{border:none;background:transparent;color:var(--mut,#a69d8c);font-weight:700;font-size:12.5px;padding:7px 15px;border-radius:9px;cursor:pointer}',
    '.ap-seg button.on{background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));color:#fff;box-shadow:0 3px 10px rgba(23,43,77,.2)}',
    ...
    '.ap-verify{font-size:12px;font-weight:700;color:var(--a1,#6fbf95);background:rgba(18,181,160,.1);border:1px solid rgba(18,181,160,.35);padding:6px 12px;border-radius:20px;white-space:nowrap;cursor:pointer}',
    '.ap-bigsimple{font-size:50px;font-weight:800;line-height:1.05;letter-spacing:-1px;margin:8px 0 6px;background:linear-gradient(135deg,var(--a1,#6fbf95),var(--a2,#2f6ef0));-webkit-background-clip:text;background-clip:text;color:transparent}',
    '.ap-plain{color:var(--mut,#a69d8c);font-size:13.5px}',
    ...
    '.ap-mrow{display:flex;justify-content:space-between;font-size:12.5px;color:var(--mut,#a69d8c);padding:7px 0;border-top:1px solid var(--line,rgba(255,255,255,.1))}',
    ...
    '.ap-tag.best{background:var(--pos,#63c08e);color:#fff;border:none}',
    '.ap-expnote{margin-top:16px;text-align:center;color:var(--mut,#a69d8c);font-size:13px;background:var(--card,rgba(255,255,255,.03));border:1px dashed var(--line,rgba(255,255,255,.18));border-radius:14px;padding:13px;cursor:pointer}',
  ].join('\n')
...
function apMapMount() {
  ...
  const s = apSubject(); if (!s || s.lat == null || s.lng == null) { el.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--mut,#a69d8c);font-size:12px">' + osIcon('map') + ' El mapa aparece al buscar el subject (lat/long de RentCast)</div>'; return; }
  apLeaflet(() => {
...
// ─── piezas de UI ───
function apConfColor(nivel) { return nivel === 'alta' ? 'var(--pos,#63c08e)' : nivel === 'media' ? 'var(--amber,#dca94f)' : 'var(--neg,#e4756a)'; }
function apStat(lbl, val, key, opts) {
  ...
  return '<div class="ap-stat">' + inner + '<div class="t">' + lbl + (opts.falta ? ' <span title="RentCast no lo trajo — cargalo" style="color:var(--amber,#dca94f);font-weight:700">· falta</span>' : '') + '</div></div>';
}
...
function apSubjStrip(s) {
  ...
  return '<div class="ap-card ap-strip" style="padding:16px 18px;margin-bottom:14px">'
    ...
    + '<div style="flex:1;min-width:220px"><div style="font-size:17px;font-weight:800">' + AP_E((fi.dirRc || s.dir || '—').split(',')[0]) + ' <span style="font-weight:500;color:var(--mut,#a69d8c);font-size:13px">' + AP_E((fi.subdivision ? fi.subdivision + ', ' : '') + (s.zip || '')) + '</span></div>'
    + '<div class="ap-plain" style="margin-top:3px;font-size:12.5px">' + AP_E(facts || 'buscá la dirección para traer los datos') + '</div></div>'
    + (esLaCasa ? '<span class="ap-verify">✓ Esta es tu casa</span>' : '<span class="ap-verify" style="color:var(--amber,#dca94f);background:rgba(231,182,94,.1);border-color:rgba(231,182,94,.35)" onclick="document.getElementById(\'ap-dir\').focus();document.getElementById(\'ap-dir\').select()">¿No es esta? corregila</span>')
    + '</div>';
}
function apHeroSimple(rec, inp) {
  ...
  const izq = '<div class="ap-card" style="padding:20px 22px' + (prov ? ';border-color:rgba(231,182,94,.45)' : '') + '">'
    ...
    + (rec && rec.arv
      ? '<div class="ap-bigsimple" style="' + (prov ? 'opacity:.65' : '') + '">' + AP_M(rec.arv) + (prov ? ' <span style="font-size:13px;color:var(--amber,#dca94f);font-weight:700">PROVISIONAL</span>' : '') + '</div>'
      + '<div class="ap-plain">Mediana ponderada de <b>' + rec.usables.length + ' casas parecidas</b> vendidas cerca, ajustadas a la tuya.' + (rec.relajado ? ' <span style="color:var(--amber,#dca94f)">(búsqueda expandida por pocas ventas)</span>' : '') + '</div>'
  ...
  let sem;
  if (!oferta) sem = { ico: '⚪', word: 'Esperando el valor', color: 'var(--mut,#a69d8c)', txt: 'buscá el subject para calcular la oferta máxima' };
  else if (!compra) sem = { ico: kitStatusDot('warn'), word: 'Falta el precio de compra', color: 'var(--amber,#dca94f)', txt: 'cargalo en Del Negocio para el veredicto' };
  else if (compra <= oferta) sem = { ico: kitStatusDot('ok'), word: 'Buen deal', color: 'var(--pos,#63c08e)', txt: 'comprás ' + AP_M(oferta - compra) + ' por debajo del máximo' };
  else if (compra <= oferta * (1 + tol)) sem = { ico: kitStatusDot('warn'), word: 'Justo al límite', color: 'var(--amber,#dca94f)', txt: 'estás ' + AP_M(compra - oferta) + ' arriba del máximo — negociá' };
  else sem = { ico: kitStatusDot('bad'), word: 'Caro para ganar', color: 'var(--neg,#e4756a)', txt: 'pagás ' + AP_M(compra - oferta) + ' más que el máximo para ganar' };
...
function apCompsSimple(s, comps, rec, f) {
  if (!comps.length) return '<div class="ap-card" style="padding:26px;text-align:center;color:var(--mut,#a69d8c)">Sin comps todavía — buscá la dirección arriba.</div>';
  const st = apState();
  ...
  const cards = us.map(x => {
    ...
    return '<div class="ap-comp ap-comp2" id="' + apCompCssId(c.id) + '">'
      ...
      + '<span style="position:absolute;right:8px;bottom:6px;font-size:9px;color:var(--mut,#a69d8c)">foto no disponible (RentCast)</span></div>'
      ...
      + '<label style="display:flex;align-items:center;gap:6px;color:var(--mut,#a69d8c);cursor:pointer"><input type="checkbox" class="ap-cbx" ' + (!excl ? 'checked' : '') + ' onchange="apExclToggle(\'' + c.id + '\')"> usar</label>'
...
function apVistaFicha(s) {
  ...
  return '<div class="ap-card">'
    + '<div class="ap-lab">Propiedad (subject) · ' + (s.rcLoading ? 'RentCast…' : s.rcOk ? '✓ RentCast' : '<span style="color:var(--amber,#dca94f)">sin RentCast — datos a mano</span>') + (s.zip ? ' · zip ' + s.zip : '') + '</div>'
    + '<div style="font-size:20px;font-weight:700;margin:6px 0 4px">' + AP_E((fi.dirRc || s.dir || '').split(',')[0] || '—') + '</div>'
    + (fi.dirRc && s.dir && fi.dirRc.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 10) !== s.dir.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 10) ? '<div style="font-size:11px;color:var(--neg,#e4756a);margin-bottom:6px">' + osIcon('alert') + ' RentCast devolvió otra dirección — verificá</div>' : '')
...
function apVistaHero(rec, inp) {
  if (!rec || !rec.arv) return '<div class="ap-card"><div class="ap-lab">ARV estimado (comps ajustados)</div><div style="color:var(--mut,#a69d8c);padding:26px 0;text-align:center">Buscá el subject para reconciliar comps.</div></div>';
  ...
  return '<div class="ap-card">'
    ...
    + (arvOfi ? '<div class="ap-plain" style="font-size:11px;margin-top:10px;padding-top:8px;border-top:1px solid var(--line,rgba(255,255,255,.1))">ARV <b>oficial</b> vigente del análisis: <b style="color:var(--ink,#eaf0ff)">' + AP_M(arvOfi) + '</b>'
        ...
        + (inp.arv_manual ? ' · <span style="color:var(--amber,#dca94f)">manual: ' + AP_E(inp.arv_fuente || 'ajustado') + '</span>' : '') + '</div>' : '')
    + '</div>';
}
...
function apCardComp(x, esRec) {
  ...
  return '<div class="ap-comp' + (excl || !x.filtro.pasa ? ' off' : '') + '" id="' + cssId + '">'
    + '<div class="ap-photo">' + osIcon('house') + '' + (c.status ? '<span class="ap-status">' + AP_E(c.status) + '</span>' : '') + '<span style="position:absolute;left:8px;bottom:6px;font-size:10px;color:var(--mut,#a69d8c)">foto: no disponible en RentCast</span></div>'
    ...
    + '<label style="display:flex;gap:5px;align-items:center;font-size:10px;color:var(--mut,#a69d8c);white-space:nowrap"><input type="checkbox" class="ap-cbx" ' + (!excl ? 'checked' : '') + ' onchange="apExclToggle(\'' + c.id + '\')"> incluir</label></div>'
    + '<div style="font-size:10.5px;color:var(--mut,#a69d8c);margin:1px 0 7px">' + (c.dist != null ? c.dist.toFixed(2) + ' mi' : '') + ' · ' + AP_D(c.fecha) + (c.dom ? ' · DOM ' + c.dom : '') + (x.filtro.pasa ? '' : ' · <span style="color:var(--amber,#dca94f)">filtrado: ' + AP_E(x.filtro.razones.join('; ')) + '</span>') + '</div>'
    + '<div style="display:flex;justify-content:space-between;align-items:baseline"><div style="font-size:22px;font-weight:800">' + AP_M(c.price) + '</div><div style="font-size:11px;color:var(--mut,#a69d8c)">' + (c.sqft && c.price ? '$' + Math.round(c.price / c.sqft) + '/sqft' : '') + '</div></div>'
    + '<div style="font-size:11.5px;color:var(--mut,#a69d8c);margin:4px 0 9px">' + (c.sqft ? c.sqft.toLocaleString() + ' sqft' : 'sqft ') + ' · ' + (c.beds != null ? c.beds : kitStatusDot('warn')) + ' cm / ' + (c.baths != null ? c.baths : kitStatusDot('warn')) + ' bñ · ' + (c.year || kitStatusDot('warn')) + ' · lote ' + (c.lot ? c.lot.toLocaleString() : kitStatusDot('warn')) + ' · piscina ' + kitStatusDot('warn') + '</div>'
    + '<div style="border-top:1px solid var(--line,rgba(255,255,255,.08));padding-top:7px">' + adjRows + '</div>'
    + '<details class="ap-man" style="margin:6px 0"><summary style="cursor:pointer;font-size:10.5px;color:var(--mut,#a69d8c)">± ajustes manuales (condición · ubicación · concesiones · pool/garaje/otros)</summary>'
    ...
    + '<div><div style="font-size:10px;color:var(--mut,#a69d8c)">valor ajustado · net ' + adj.netPct.toFixed(1) + '% · gross ' + adj.grossPct.toFixed(1) + '%' + (adj.grossPct > apCfg('arv_gross_adj_warn_pct', 25) ? osIcon('alert') : '') + '</div><div style="font-size:17px;font-weight:800">' + AP_M(adj.valorAjustado) + '</div></div>'
...
function apVistaGrilla(s, rec) {
  if (!rec || !rec.usables || !rec.usables.length) return '<div class="ap-card" style="text-align:center;color:var(--mut,#a69d8c)">Sin comps reconciliados.</div>';
  ...
  const head = '<tr><th>Comparable →</th><th class="subjcol">SUBJECT<div style="font-weight:500;font-size:10px;color:var(--mut,#a69d8c)">' + AP_E((s.dir || '').split(',')[0]) + '</div></th>'
    + us.map(x => '<th><div style="font-weight:700">' + AP_E(x.c.dir.split(',')[0]) + '</div><div style="font-weight:500;font-size:10px;color:var(--mut,#a69d8c)">' + (x.c.dist != null ? x.c.dist.toFixed(2) + ' mi · ' : '') + AP_D(x.c.fecha) + '</div></th>').join('') + '</tr>';
...
function ffArvProView() {
  ...
  if (modo === 'simple') {
    const mapaS = '<div class="ap-sectitle">' + osIcon('map-pin') + ' Dónde están las casas comparables</div>'
      ...
      + '<div style="position:absolute;left:12px;top:10px;z-index:500;font-size:11px;color:var(--mut,#a69d8c);background:var(--card,rgba(10,14,20,.75));padding:4px 9px;border-radius:8px;border:1px solid var(--line,rgba(255,255,255,.1))">TU CASA + comps · click en un pin resalta la tarjeta</div></div>';
  ...
  const mapa = '<div style="position:relative;margin-bottom:4px"><div id="ap-map"></div>'
    + '<div style="position:absolute;left:12px;top:10px;z-index:500;font-size:11px;color:var(--mut,#a69d8c);background:var(--card,rgba(10,14,20,.75));padding:4px 9px;border-radius:8px;border:1px solid var(--line,rgba(255,255,255,.1))">' + osIcon('map-pin') + ' Subject + ' + comps.length + ' comparables · click en un pin resalta el comp</div></div>';
  ...
  let compsHtml;
  if (!comps.length) compsHtml = '<div class="ap-card" style="text-align:center;padding:30px;color:var(--mut,#a69d8c)">Sin comps todavía — buscá el subject arriba.</div>';
  ...
  const foot = rec && rec.arv ? '<div style="display:flex;gap:12px;align-items:center;margin:14px 0;flex-wrap:wrap"><button class="ap-btn" onclick="apUsarArv(' + rec.arv + ')">→ Usar ' + AP_M(rec.arv) + ' como ARV del análisis</button>'
    + '<span style="font-size:11.5px;color:var(--mut,#a69d8c)">' + (rec.usables[0] ? 'El más parecido: ' + AP_E(rec.usables[0].c.dir.split(',')[0]) + ' (gross ' + rec.usables[0].adj.grossPct.toFixed(1) + '% → peso ' + rec.usables[0].pesoPct + '%).' : '') + ' El ARV de Airtable sigue siendo la fuente de verdad guardada.</span></div>' : '';

  const fEd = (k, l, def) => '<div style="margin-bottom:8px"><div style="font-size:10.5px;color:var(--mut,#d9d1c2);font-weight:600;margin-bottom:2px">' + l + '</div><input value="' + apCfg(k, def) + '" onchange="apCfgSet(\'' + k + '\',this.value)" style="width:100%;background:var(--glass,rgba(255,255,255,.05));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:8px;padding:6px 9px;color:inherit;font-size:12.5px;font-weight:600;outline:none"></div>';
  const factores = '<details class="ap-card" style="padding:14px;margin-bottom:14px"><summary style="cursor:pointer" class="ap-lab">' + osIcon('settings') + ' Factores de ajuste (calibrados, editables — ff_uw_config)</summary>'
    ...
    + '</div><div style="font-size:10px;color:var(--mut,#a69d8c);margin-top:4px">Guías de piscina/garaje/fireplace: RentCast no dice si el COMP los tiene → usá el campo manual "otros" del comp con estos montos.</div></details>';
  ...
  const calHtml = '<div class="ap-card ap-grid" style="padding:14px"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:8px">'
    ...
    + (cal.sugerenciaBias != null && Math.abs(cal.sugerenciaBias) > 0.5 && Math.abs(apCfg('arv_bias_pct', 0) - cal.sugerenciaBias) > 0.5
      ? '<div style="background:rgba(231,182,94,.08);border:1px solid rgba(231,182,94,.3);border-radius:9px;padding:8px 12px;margin-bottom:8px;font-size:11.5px">' + osIcon('lightbulb') + ' Sugerencia de recalibración: aplicar sesgo <b>' + cal.sugerenciaBias.toFixed(2) + '%</b> al reconciliado (hoy ' + apCfg('arv_bias_pct', 0) + '%). '
    ...
    + '<div style="font-size:10px;color:var(--mut,#a69d8c);margin-top:6px">Excluidas de la calibración (atípicos/datos sucios, editable en arv_calib_excluir): ' + AP_E(window.UWct ? UWct('arv_calib_excluir', '—') : '—') + '</div></div>';
