...
function ffUwDefaults() {
  return {
    ...
    // MODO VENTA (flip) — cascada de experto: precio − costos venta − all-in − interés HML − holding = utilidad
    venta_precio: 0,                                            // 0 = usar arv.probable (precio de venta esperado, editable)
    venta_comision_pct: UWc('venta_comision_pct', 6),          // comisión de venta (% del precio)
    venta_cierre_pct: UWc('venta_cierre_pct', 1.5),            // cierre vendedor: title/escrow/transfer (% del precio)
    venta_concesiones_pct: UWc('venta_concesiones_pct', 0),   // concesiones al comprador (% del precio)
    venta_staging: UWc('venta_staging', 0),                    // staging / preparación ($, opcional)
    venta_capital: 0,                                          // 0 = usar negocio.cashToClose (down + cierre compra + no cubierto por HML)
    venta_payoff: 0,                                           // 0 = HML real del deal (saldo Airtable) o ⛓ modelado
    venta_meses: 0,                                            // meses hasta vender (0 = derivar del hold) → carry HML + ROI anualizado
    venta_holding_mes: 0,                                     // holding $/mes (0 = utilities + seguro + predial derivados)
    venta_impuesto_pct: UWc('venta_impuesto_pct', 0),         // impuesto estimado sobre la utilidad (flip = ordinaria; editable)
    venta_split_inv_pct: UWc('venta_split_inversionista_pct', 50),
    venta_dias: 0,                                            // legacy: días del proyecto (se convierte a meses)
...
//   precio − comisión − cierre vendedor − concesiones − ALL-IN (compra+rehab) − INTERÉS HML total
//   (solo interés × meses hasta vender) − holding (utilities+seguro+predial) − staging = UTILIDAD NETA.
//   El payoff del HML NO entra en la utilidad (el préstamo es financiación del all-in); sí en el Net Wire
//   (cash en la mesa = precio − costos de venta − payoff). Encadena con Del Negocio (all-in, capital) e
//   Intereses (pago mensual HML). Verificado contra el ejemplo del CEO: $449,177 → utilidad $65,638.
function ffUwCalcVenta(inp, arv, negocio, intereses) {
  const R2 = n => Math.round(n * 100) / 100;
  const precio = +inp.venta_precio > 0 ? +inp.venta_precio : (arv.probable || 0);   // precio de venta = ARV por default (editable)
  const esArv = !(+inp.venta_precio > 0);
  // ── COSTOS DE VENTA (% del precio) ──
  const comision = R2(precio * ((+inp.venta_comision_pct || 0) / 100));                     // default 6%
  const cierre = R2(precio * ((+inp.venta_cierre_pct || 0) / 100));                         // title/escrow/transfer vendedor, default 1.5%
  const concesiones = R2(precio * ((+inp.venta_concesiones_pct || 0) / 100));               // al comprador, default 0%
  const costosVenta = R2(comision + cierre + concesiones);
  // ── PAYOFF del HML (se cancela en el cierre; NO hay refi) → para el Net Wire ──
  ...
  const netWire = R2(precio - costosVenta - payoff);                                        // lo que te giran en la mesa de cierre
  // ── TIEMPO hasta vender (meses) — driver del carry HML y del ROI anualizado ──
  const meses = +inp.venta_meses > 0 ? +inp.venta_meses
    : (+inp.venta_dias > 0 ? Math.round(+inp.venta_dias / 30.44 * 10) / 10 : (negocio.mesesHold > 0 ? negocio.mesesHold : null));
  const mesesEstimado = !(+inp.venta_meses > 0);
  // ── COSTOS DEL PROYECTO (economía real del flip, NO el payoff del préstamo) ──
  const allIn = R2((+inp.purchase || 0) + (negocio.remod || 0));                            // compra + rehab (Total Draws)
  const intMensualHml = (intereses && intereses.intMensualHarmony) || 0;                    // ⛓ Intereses (solo interés, sin DSCR)
  const interesHmlTotal = R2(intMensualHml * (meses || 0));                                 // el carry real que se come la utilidad
  const seguroMes = UWc('seguro_mensual', 120);
  const predialMes = R2((arv.probable || 0) * (UWc('impuestos_pct_arv', 2.2) / 100) / 12);
  const holdingMesCalc = R2((+inp.utilities_mes || 0) + seguroMes + predialMes);
  const holdingMes = +inp.venta_holding_mes > 0 ? +inp.venta_holding_mes : holdingMesCalc;  // editable (0 = derivado)
  const holdingMesEsCalc = !(+inp.venta_holding_mes > 0);
  const holding = R2(holdingMes * (meses || 0));
  const staging = +inp.venta_staging || 0;
  // ── UTILIDAD del proyecto (antes de impuestos) ──
  const utilidadAntesImp = R2(precio - costosVenta - allIn - interesHmlTotal - holding - staging);
  const impuestoPct = +inp.venta_impuesto_pct || 0;                                         // flip = ganancia ORDINARIA (editable, default 0)
  const impuesto = (impuestoPct > 0 && utilidadAntesImp > 0) ? R2(utilidadAntesImp * impuestoPct / 100) : 0;
  const utilidad = R2(utilidadAntesImp - impuesto);
  // ── MÉTRICAS del flip ──
  const margen = precio > 0 ? Math.round(10000 * utilidad / precio) / 100 : null;           // % sobre venta
  const capital = +inp.venta_capital > 0 ? +inp.venta_capital : (negocio.cashToClose || 0); // down + cierre compra + lo no cubierto por HML
  const capitalEsCtc = !(+inp.venta_capital > 0);
  const roi = capital > 0 ? Math.round(10000 * utilidad / capital) / 100 : null;            // cash-on-cash
  const roiAnual = (roi != null && meses > 0) ? Math.round(100 * roi * 12 / meses) / 100 : null;  // ★ velocidad manda
  // ── CHECK regla 70% / MAO: all-in ≤ ARV × maxPct − costos de venta ──
  const maoPct = UWc('allin_max_pct', 75);
  const maoVenta = (arv.probable || 0) > 0 ? R2(arv.probable * maoPct / 100 - costosVenta) : null;
  const regla70 = maoVenta != null ? allIn <= maoVenta : null;
  // ── REPARTO: devolver capital a inversionistas → repartir la utilidad por % equity ──
  ...
  return { precio, esArv, comision, cierre, concesiones, costosVenta, payoff, payoffFuente, netWire,
    meses, mesesEstimado, allIn, intMensualHml, interesHmlTotal, seguroMes, predialMes, holdingMes, holdingMesEsCalc, holding, staging,
    utilidadAntesImp, impuestoPct, impuesto, utilidad, margen, capital, capitalEsCtc, roi, roiAnual,
...
function ffUwComputeFor(inp) {
  ...
  const venta = ffUwCalcVenta(inp, arv, negocio, intereses);   // ⛓ el interés mensual del HML alimenta el carry de la venta
  ...
  if (esVenta) {
    // MODO VENTA: la ganancia es UNA vez (utilidad del flip). Guardrails: utilidad+, regla 70%, sin déficit.
    const gUtil = venta.utilidad > 0;
    const g70 = venta.regla70 !== false;                       // null (sin ARV) no penaliza
    const go = gUtil && g70 && gDeficit;
    const veredicto = go ? 'GO' : (gUtil && (g70 || gDeficit)) ? 'revisar' : 'NO-GO';
    return { negocio, arv, cashout, intereses, ingreso, venta,
      unificada: { modo: 'venta', allIn, allInPct, allInMax, mao, cashToClose: negocio.cashToClose,
        precio: venta.precio, netWire: venta.netWire, utilidad: venta.utilidad, margen: venta.margen,
        parteInv: venta.parteInv, parteOp: venta.parteOp, splitInvPct: venta.splitInvPct,
        roi: venta.roi, roiAnual: venta.roiAnual, meses: venta.meses, regla70: venta.regla70, maoVenta: venta.maoVenta,
        gAllIn, gDeficit, gUtil, g70, veredicto } };
  }
...
function ffUwShell() {
  ...
  const sel = `<select onchange="if(this.value==='__new')ffUwNuevo(false);else if(this.value==='__hyp')ffUwNuevo(true);else if(this.value.startsWith('deal:'))ffUwCargarCasa(this.value.slice(5));else if(this.value.startsWith('an:'))ffUwAbrir(this.value.slice(3));this.value='__keep'" style="background:var(--card,rgba(255,255,255,.05));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:9px;padding:8px 11px;color:inherit;font-size:13px;min-width:230px">
    ...
    <option value="__hyp">${osIcon('flask')} Casa hipotética</option>
  ...
  const estToggle = a ? `<div style="display:inline-flex;background:var(--glass,rgba(255,255,255,.06));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:11px;padding:3px;gap:2px" title="La estrategia genera las entradas y salidas del análisis">
    ${[['venta', 'Vender (flip)', 'salida = vender al ARV'], ['hold', 'Rentar (hold)', 'salida = refinanciar y quedársela']].map(([k, lbl, tip]) => `<button onclick="ffUwSetEstrategia('${k}')" title="${tip}" style="border:none;border-radius:8px;padding:6px 12px;font-size:12px;font-weight:${estr === k ? '700' : '500'};cursor:pointer;background:${estr === k ? 'linear-gradient(135deg,var(--a1,#5c79f0),var(--a2,#3a5be0))' : 'transparent'};color:${estr === k ? '#fff' : 'inherit'}">${lbl}</button>`).join('')}
  </div>` : '';
  const nav = a ? `<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:14px 0">${estToggle}<div style="display:flex;gap:4px;flex-wrap:wrap">${ffUwNav().map(([k, i, n]) => `<button onclick="ffUwSub('${k}')" style="padding:8px 13px;border-radius:9px;border:1px solid ${UW.sub === k ? 'var(--a1,#5c79f0)' : 'var(--line,rgba(255,255,255,.12))'};background:${UW.sub === k ? 'rgba(92,121,240,.12)' : 'transparent'};color:inherit;cursor:pointer;font-size:12px;font-weight:${UW.sub === k ? '700' : '500'}">${i} ${n}</button>`).join('')}</div></div>` : '';
  const body = a ? ffUwSubBody() : `<div class="card" style="text-align:center;padding:40px"><div style="font-size:42px;margin-bottom:10px">${osIcon('chart')}</div><h2 style="margin-bottom:8px">Suite de Underwriting</h2><p style="color:var(--txt3,#8b93a1);font-size:13px;margin-bottom:16px">Elegí la estrategia y el análisis genera su flujo · calibrada con ${UW.deals.length} casas reales</p><div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap"><button class="repbtn" onclick="ffUwNuevo(false,'venta')">${osIcon('clipboard')} Nuevo · Vender (flip)</button><button class="repbtn" onclick="ffUwNuevo(false,'hold')">${osIcon('house')} Nuevo · Rentar (hold)</button><button class="repbtn ghost" onclick="ffUwNuevo(true,'venta')">${osIcon('flask')} Hipotética</button></div></div>`;
  return `<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">${sel}${a ? `<input value="${UW_E(a.nombre)}" onchange="UW.a.nombre=this.value" style="background:var(--card,rgba(255,255,255,.05));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:9px;padding:8px 11px;color:inherit;font-size:13px" placeholder="nombre del análisis"><button class="repbtn" data-uw-guardar="1">${osIcon('save')} Guardar</button><span style="font-size:11px;opacity:.6">${a.ff_deal_id ? 'casa real' : a.es_hipotetica ? 'hipotética' : ''} · calibrado con ${UW.deals.length} casas</span>` : ''}</div>${nav}${body}`;
}
...
function UW_IN(lab, k, val, opts) {
  ...
  return '<div style="margin-bottom:10px"><div style="font-size:11.5px;color:var(--txt2,#c7cdd8);font-weight:600;margin-bottom:3px">' + lab + (opts.help ? '<span title="' + UW_E(opts.help) + '" style="opacity:.45;margin-left:5px;cursor:help;font-size:11px">&#9432;</span>' : '') + '</div><div style="display:flex;align-items:center;background:var(--card,rgba(255,255,255,.05));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:9px;padding:0 10px">' + (pre ? '<span style="opacity:.5;font-size:13px">' + pre + '</span>' : '') + '<input value="' + shown + '" placeholder="' + ph + '" onchange="ffUwSet(&quot;' + k + '&quot;,this.value.replace(/[,%\\s' + UW_DLR + ']/g,&quot;&quot;))" style="flex:1;background:none;border:none;padding:8px 6px;color:inherit;font-size:14px;font-weight:600;outline:none">' + (suf ? '<span style="opacity:.5;font-size:13px">' + suf + '</span>' : '') + '</div>' + (opts.hint ? '<div style="font-size:10px;color:var(--txt3,#8b93a1);margin-top:2px">' + opts.hint + '</div>' : '') + '</div>';
}
function UW_HERO(titulo, valor, sub, color) {
  const c = color || 'var(--a1,#5c79f0)';
  return '<div style="background:linear-gradient(135deg,' + c + '22,transparent);border:1px solid ' + c + '55;border-radius:14px;padding:18px 22px;margin-bottom:16px"><div style="font-size:12px;color:var(--txt2,#c7cdd8);font-weight:600;text-transform:uppercase;letter-spacing:.5px">' + titulo + '</div><div style="font-size:40px;font-weight:800;color:' + c + ';line-height:1.1;margin:4px 0">' + valor + '</div>' + (sub ? '<div style="font-size:12px;color:var(--txt3,#8b93a1)">' + sub + '</div>' : '') + '</div>';
}
function UW_BLOCK(subtitulo, inner) {
  return '<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.6px;color:var(--a1,#5c79f0);margin-bottom:8px;padding-bottom:4px;border-bottom:1px solid var(--line,rgba(255,255,255,.08))">' + subtitulo + '</div>' + inner + '</div>';
}
function UW_ROW(l, v, cls) {
  return '<div style="display:flex;justify-content:space-between;padding:5px 0;font-size:13px' + (cls === 'tot' ? ';font-weight:700;border-top:1px solid var(--line,rgba(255,255,255,.12));margin-top:6px;padding-top:8px' : '') + '"><span style="color:var(--txt2,#c7cdd8)">' + l + '</span><b class="' + (cls && cls !== 'tot' ? cls : '') + '">' + UW_M(v) + '</b></div>';
}
function UW_CARD(titulo, proposito, inner) {
  return '<div class="card" style="padding:20px"><div style="margin-bottom:14px"><div style="font-size:17px;font-weight:700">' + titulo + '</div><div style="font-size:12px;color:var(--txt3,#8b93a1);margin-top:2px">' + proposito + '</div></div>' + inner + '</div>';
}
...
function ffUwViewNegocio() {
  ...
  const cal = UW.calib;
  const supuestosO7 = cal ? '<div style="background:rgba(92,121,240,.07);border:1px solid rgba(92,121,240,.28);border-radius:10px;padding:10px 12px;margin-top:10px;font-size:11.5px">'
    + '' + osIcon('settings') + ' <b>Historia real</b>: obra <b>$' + (cal.psf_real || '—') + '/sqft</b> (' + cal.psf_n + ' finalizadas) · duración <b>' + (cal.obra_meses_prom || '—') + ' m</b> · holding hasta rentar <b>' + (cal.holding_meses_prom || '—') + ' m</b>. Hoy: $/sqft media <b>$' + ffUwPsf('media') + '</b> · hold <b>' + r.mesesHold + ' m</b> (default = obra calibrada).'
  ...
  const hero = kitHero('El inversionista pone', UW_M2(r.cashToClose),
    'down + gastos de cierre &minus; créditos (HUD) · Draw al Harmony: <b>' + UW_M(r.draw) + '</b>' + (r.ctcReal ? ' · real Airtable ' + UW_M(r.ctcReal) + ' (Δ ' + UW_M(r.cashToClose - r.ctcReal) + ')' : ''), 'var(--a2,#3a5be0)');
  const inputs = '<div class="card" style="padding:20px 22px;margin-bottom:16px;border-radius:18px">'
    ...
    + '</div>'
    + '<div style="font-size:11px;color:var(--txt3,#8b93a1);margin:-4px 0 10px">típico Harmony: <b>90%</b> de la compra · <b>100%</b> de la remo — préstamo = ' + r.pctCompra + '%×compra + ' + r.pctRemo + '%×' + (r.baseModo === 'rehab' ? 'rehab' : 'draw') + '</div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">'
    ...
    + '</div>'
    + '<div style="font-size:11px;color:var(--txt3,#8b93a1);margin:-4px 0 10px">' + osIcon('clock') + ' obra ' + r.mesesObra + ' m + hasta rentar/vender ' + r.mesesRenta + ' m = <b>holding ' + r.mesesHold + ' m</b> — sobre ESA suma corren intereses, utilities e insurance (lo real que se paga y sale de los draws) · obra calibrada: ' + ((UW.calib && UW.calib.obra_meses_prom) || '—') + ' m</div>'
    + '</div>';
  const adv = '<details ' + (UW.negAdv ? 'open' : '') + ' ontoggle="UW.negAdv=this.open" style="background:var(--card,rgba(255,255,255,.04));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:18px;padding:4px 22px;margin-bottom:16px">'
    + '<summary style="cursor:pointer;font-size:13px;font-weight:600;color:var(--txt3,#8b93a1);padding:14px 0;list-style:none">' + osIcon('settings') + '︎ Ajustes (opcional) — draw y cierre HUD, ya calibrados</summary>'
    + '<div style="padding-bottom:16px">'
    + '<div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--a1,#5c79f0);margin:4px 0 8px">Del draw</div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">'
    ...
    + '</div>'
    + '<div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--a2,#3a5be0);margin:16px 0 8px">Cierre HUD · fees prestamista</div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">'
    ...
    + '</div>'
    + '<div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--pos,#4ade9e);margin:16px 0 8px">Créditos al cierre (ya pagados, se acreditan)</div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">'
    ...
    + '</div>'
    + '<div style="margin-top:14px;display:flex;gap:8px;align-items:center"><button class="repbtn ' + (inp.wholesale ? '' : 'ghost') + '" style="padding:5px 11px;font-size:11px" onclick="ffUwSet(\'wholesale\',' + (inp.wholesale ? 'false' : 'true') + ')">' + (inp.wholesale ? osIcon('check-circle') : '☐') + ' Deal por wholesaler</button>'
    + (inp.wholesale ? '<div style="flex:1">' + kitInputSm('Assignment fee', inp.cc_assignment, "ffUwSet('cc_assignment',VAL)") + '</div>' : '<span style="font-size:10px;color:var(--txt3,#8b93a1)">assignment fee al cierre (Bethune: $40,000)</span>') + '</div>'
    + '</div></details>';
  const desglose = '<div class="card" style="padding:20px 22px;border-radius:18px">'
    + '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1);margin-bottom:4px">Draw al Harmony</div>'
    + kitRow('Remodelación', r.remod) + kitRow(+inp.intereses_override > 0 ? 'Intereses HML (MANUAL — <a style="cursor:pointer;color:var(--a1,#5c79f0)" onclick="ffUwSet(\'intereses_override\',0)">↩ volver al calculado</a>)' : 'Intereses HML (' + r.mesesHold + 'm de HOLDING @ ' + r.tasaHml + '%/año · solo interés · sobre el préstamo bruto)', r.intereses) + kitRow('Utilities (' + r.mesesHold + 'm de holding)', r.utilities)
    ...
    + kitRow('<b>= Draw total</b>', r.draw, { big: true, color: 'var(--a1,#5c79f0)' })
    + (+inp._draw_real > 0 ? '<div style="font-size:10.5px;color:var(--txt3,#8b93a1);margin-top:4px">⛓ Draw real del deal (Airtable): ' + UW_M(+inp._draw_real) + ' &middot; &Delta; modelo ' + UW_M(r.draw - inp._draw_real) + '</div>' : '')
    + alerta + supuestosO7
    + '<div style="border-top:1px solid var(--line,rgba(255,255,255,.12));margin-top:10px;padding-top:12px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1)">El inversionista pone (HUD)</div>'
    ...
    + kitRow('<b>= EL INVERSIONISTA PONE</b>', null, { big: true, txt: UW_M2(r.cashToClose), color: 'var(--a2,#3a5be0)', last: true })
    + '<div style="font-size:11px;color:var(--txt2,#c7cdd8);margin-top:8px;background:color-mix(in srgb, var(--a2) 8%, transparent);border-radius:8px;padding:8px 10px">Ya pagado como earnest/option: <b>' + UW_M2(r.yaPagado) + '</b> (acreditado) &middot; falta al cierre: <b>' + UW_M2(r.cashToClose) + '</b></div>'
    + (r.closingReal ? '<div style="font-size:10px;color:var(--txt3,#8b93a1);margin-top:6px">Gastos de cierre reales (Airtable): ' + UW_M(r.closingReal) + ' &middot; &Delta; ' + UW_M(r.closing - r.closingReal) + '</div>' : '')
    // 1C · lo que RECIBE el negocio vs lo que DEBE al refinanciar — dos números distintos (punto 1)
    + '<div style="border-top:1px solid var(--line,rgba(255,255,255,.12));margin-top:12px;padding-top:12px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1)">El hard money te presta (desembolso neto)</div>'
    ...
    + kitRow('<b>= Desembolso neto al negocio</b>', null, { big: true, txt: UW_M2(r.desembolsoNeto), color: 'var(--pos,#4ade9e)' })
    + '<div style="border-top:1px solid var(--line,rgba(255,255,255,.12));margin-top:12px;padding-top:12px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1)">Payoff del HML al refinanciar</div>'
    + kitRow('Principal del HML', r.prestamo)
    + kitRow('+ Interés/fees que se capitalizan (term sheet)', r.capitaliza || 0)
    + kitRow('<b>= Lo que le pagás al refinanciar</b>', null, { big: true, txt: UW_M2(r.payoffHml), color: 'var(--amber,#fbbf24)', last: true })
    + '<div style="font-size:10.5px;color:var(--txt3,#8b93a1);margin-top:6px">⛓ Este payoff alimenta SOLO la Calc 3 (Cash-Out) — no lo re-tecleás. Recibís ' + UW_M2(r.desembolsoNeto) + ' pero debés ' + UW_M2(r.payoffHml) + ': no son lo mismo.</div>'
    + '</div>';
  return '<div style="max-width:560px;margin:0 auto">'
    + '<div style="margin-bottom:16px"><div style="font-size:19px;font-weight:700">Del Negocio</div><div style="font-size:13px;color:var(--txt3,#8b93a1)">Compra + obra: cuánto desembolsa el prestamista y cuánto pone el inversionista.</div></div>'
...
function ffUwViewArv() {
  ...
  const hero = UW_HERO('ARV (valor de reventa)', UW_M(a.probable), a.esAirtable ? 'de Airtable (fuente de verdad) &middot; confianza ' + a.confianza : 'estimado por comps &middot; confianza ' + a.confianza, a.esAirtable ? 'var(--pos,#4ade9e)' : 'var(--amber,#fbbf24)');
  const izq = UW_CARD('2 &middot; ARV', 'El valor de la casa remodelada. La fuente de verdad es el ARV de Airtable.',
    ...
    UW_BLOCK('Referencia por comparables', UW_IN('Superficie (sqft)', 'est_sqft', inp.est_sqft, { tipo: 'num' }) + '<div style="font-size:11px;color:var(--txt3,#8b93a1)">$/sqft de la zona: $' + a.psfZona + ' &rarr; comps = ' + UW_M(a.arvComps) + ' <b>(referencia)</b></div>' + ffUwRcCompsBox()) +
    '<div style="font-size:10px;color:var(--txt3,#8b93a1);margin-top:6px">&#128268; Mercado en vivo (PropStream/HAR): pendiente de enganche.</div>');
  const der = hero + '<div class="card" style="padding:16px"><div style="font-size:11px;font-weight:800;text-transform:uppercase;color:var(--txt3,#8b93a1);margin-bottom:8px">Rango de valor</div>' + UW_ROW('&#128317; Conservador', a.conservador) + UW_ROW('&#127919; Probable', a.probable, 'tot') + UW_ROW('&#128316; Optimista', a.optimista) + '<div style="font-size:10px;color:var(--txt3,#8b93a1);margin-top:8px">Comps (referencia): ' + UW_M(a.arvComps) + (a.appraisal > 0 ? ' &middot; appraisal: ' + UW_M(a.appraisal) : '') + '</div></div>';
...
function ffUwViewCashout() {
  ...
  const heroCol = pos ? 'var(--pos,#4ade9e)' : 'var(--neg,#ff6b6b)';
  ...
  const IN3 = (lab, hint, k, val, opts) => {
    ...
    return '<div style="margin-bottom:16px"><div style="font-size:13px;font-weight:600;margin-bottom:6px">' + lab + (hint ? ' <span style="color:var(--txt3,#8b93a1);font-weight:400;font-size:12px">— ' + hint + '</span>' : '') + '</div>'
      ...
      + (pct ? '' : '<span style="color:var(--txt3,#8b93a1);font-size:15px">' + UW_DLR + '</span>')
      ...
      + (pct ? '<span style="color:var(--txt3,#8b93a1);font-size:15px">%</span>' : '') + '</div>'
      + (opts.foot ? '<div style="font-size:11px;color:var(--txt3,#8b93a1);margin-top:4px">' + opts.foot + '</div>' : '') + '</div>';
  };
  const IN3s = (lab, k, val, opts) => { // versión compacta para "Ajustes" (grilla 2 col)
    opts = opts || {}; const pct = !!opts.pct, plain = !!opts.plain;
    return '<div><div style="font-size:12px;font-weight:600;margin-bottom:5px;color:var(--txt2,#c7cdd8)">' + lab + '</div>'
      ...
      + (pct || plain ? '' : '<span style="color:var(--txt3,#8b93a1);font-size:13px">' + UW_DLR + '</span>')
      ...
      + (pct ? '<span style="color:var(--txt3,#8b93a1);font-size:13px">%</span>' : '') + '</div></div>';
  };
  const ROW3 = (l, v, opts) => {
    opts = opts || {};
    return '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;padding:7px 0;' + (opts.last ? '' : 'border-bottom:1px dashed var(--line,rgba(255,255,255,.1));') + (opts.big ? 'font-size:15px;' : 'font-size:13px;') + 'color:var(--txt2,#c7cdd8)"><span>' + l + '</span><b style="color:' + (opts.neg ? 'var(--neg,#ff6b6b)' : opts.color || 'inherit') + ';font-weight:700;white-space:nowrap">' + (opts.txt != null ? opts.txt : UW_M2(v)) + '</b></div>';
  };
  ...
  const heroReal = c.cashoutReal != null ? '<div style="font-size:12px;margin-top:8px;opacity:.85;color:' + heroCol + '">' + osIcon('target') + ' real (Airtable): <b>' + UW_M2(c.cashoutReal) + '</b> · Δ modelo ' + UW_M2(c.cashOut - c.cashoutReal) + '</div>' : '';
  ...
  const inputs = '<div class="card" style="padding:20px 22px;margin-bottom:16px;border-radius:18px">'
    ...
    + IN3('Payoff del HML', 'saldo a pagar al refinanciar — ⛓ se llena solo desde Del Negocio', 'payoff', c.payoffOverride ? inp.payoff : c.payoff, {
      foot: 'fuente: ' + c.payoffFuente
        + (c.payoffOverride && c.payoffCalc > 0 ? ' · <a style="cursor:pointer;color:var(--a1,#5c79f0)" onclick="ffUwSet(\'payoff\',0)">↩ volver al calculado (' + UW_M2(c.payoffCalc) + ')</a>' : '')
  ...
  const adv = '<details ' + (UW.cashoutAdv ? 'open' : '') + ' ontoggle="UW.cashoutAdv=this.open" style="background:var(--card,rgba(255,255,255,.04));border:1px solid var(--line,rgba(255,255,255,.12));border-radius:18px;padding:4px 22px;margin-bottom:16px">'
    + '<summary style="cursor:pointer;font-size:13px;font-weight:600;color:var(--txt3,#8b93a1);padding:14px 0;list-style:none">' + osIcon('settings') + '︎ Ajustes (opcional) — ya vienen con tus valores calibrados</summary>'
    ...
    + '</div><div style="font-size:11px;color:var(--txt3,#8b93a1);margin-top:12px">Tasa DSCR ' + c.tasaAnual + '% · 30 años (Champions, reverseada de tus refis — editable en config)' + (c.prestamoReal > 0 ? ' · usando el préstamo REAL de Airtable ' + UW_M(c.prestamoReal) : '') + '</div></div></details>';
  ...
  const desglose = '<div class="card" style="padding:20px 22px;border-radius:18px">'
    ...
    + '<div style="border-top:1px solid var(--line,rgba(255,255,255,.12));margin-top:8px;padding-top:12px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1)">Detalle de costos</div>'
    ...
    + '<div style="background:rgba(58,91,224,.07);border-radius:10px;padding:10px 12px;margin-top:10px;font-size:12px;color:var(--txt2,#c7cdd8);line-height:1.5">' + osIcon('lightbulb') + ' De los costos, <b>' + UW_M2(c.escrows) + '</b> son escrows (impuestos + seguro): <b>tu propia plata guardada</b>, no un gasto perdido. Capital recuperado neto: <b style="color:var(--pos,#4ade9e)">' + UW_M2(c.recuperadoNeto) + '</b></div>'
    + (c.pagadoReal > 0 ? '<div style="font-size:10.5px;color:var(--txt3,#8b93a1);margin-top:8px">Pagado al HML con la refi (Airtable, incluye costos): ' + UW_M2(c.pagadoReal) + ' · modelo payoff+costos: ' + UW_M2(c.payoff + c.costos) + '</div>' : '')
    + '</div>';
  return '<div style="max-width:560px;margin:0 auto">'
    + '<div style="margin-bottom:16px"><div style="font-size:19px;font-weight:700">Refi Cash-Out</div><div style="font-size:13px;color:var(--txt3,#8b93a1)">Meté 3 datos y te da el cash-out. Lo demás ya está calibrado con tus refis reales.</div></div>'
    + hero + inputs + adv + desglose + '</div>';
}
...
function ffUwViewIntereses() {
  ...
  const amber = 'var(--amber,#fbbf24)', azul = 'var(--a2,#3a5be0)';
  ...
  const cardHml = '<div class="card" style="padding:18px 22px;border-radius:18px;margin-bottom:22px">'
    ...
    + '</div>'
    + '<div style="font-size:10.5px;color:var(--txt3,#8b93a1);margin-top:8px">Holding = obra + hasta rentar/vender (los meses solo mueven el <b>total</b>, no el pago mensual). ⛓ Propaga: el pago mensual al flujo durante el hold (Calc 5) y al carry del déficit; el total a la reserva de interés del draw (Calc 1).</div>'
    + '</div>';
  ...
  if (esVenta) {
    ...
    const carryVenta = '<div class="card" style="padding:14px 18px;border-radius:16px;margin-top:-8px;background:color-mix(in srgb, ' + amber + ' 8%, transparent)">'
      ...
      + '</div>'
      + '<div style="font-size:10.5px;color:var(--txt3,#8b93a1);margin-top:8px">⛓ Este <b>' + UW_M2(v.interesHmlTotal) + '</b> entra como COSTO en la calc de Venta (se come la utilidad). Al vender, el HML se cancela con el <b>payoff</b> en el cierre — no hay refinanciación.</div>'
      + '</div>';
    return '<div style="max-width:560px;margin:0 auto">'
      + '<div style="margin-bottom:16px"><div style="font-size:19px;font-weight:700">Intereses (venta)</div><div style="font-size:13px;color:var(--txt3,#8b93a1)">En un flip solo pagás el <b>hard money</b> durante el holding hasta vender. Nada de DSCR/refi.</div></div>'
      + heroHml + cardHml + carryVenta + '</div>';
  }
  return '<div style="max-width:560px;margin:0 auto">'
    + '<div style="margin-bottom:16px"><div style="font-size:19px;font-weight:700">Intereses</div><div style="font-size:13px;color:var(--txt3,#8b93a1)">Dos préstamos distintos, cada uno con su pago mensual: el hard money durante la obra/hold y el DSCR después del refi.</div></div>'
...
function ffUwViewIngreso() {
  ...
  const hero = kitHero('Flujo mensual', UW_M(g.flujo) + '/mes',
    (pos ? '' : '' + osIcon('alert') + ' <b>flujo negativo</b> — la renta no cubre los gastos · ') + (g.cashOnCash != null ? 'cash-on-cash ' + g.cashOnCash + '% sobre ' + UW_M(g.cashLeft) + ' que quedan invertidos' : (g.renta === 0 ? 'cargá la renta para ver el flujo real' : '')), pos ? 'var(--pos,#4ade9e)' : 'var(--neg,#ff6b6b)');
  const inputs = '<div class="card" style="padding:20px 22px;margin-bottom:16px;border-radius:18px">'
  ...
  const desglose = '<div class="card" style="padding:20px 22px;border-radius:18px">'
    ...
    + kitRow('<b>= Flujo mensual</b>', g.flujo, { big: true, color: pos ? 'var(--pos,#4ade9e)' : 'var(--neg,#ff6b6b)', last: true })
    + '</div>'
    // ⛓ pago HML de la Calc 4: antes del refi el flujo paga el hard money, no la cuota DSCR
    + (o.intereses.intMensualHarmony > 0 ? '<div class="card" style="padding:11px 16px;margin-top:10px;font-size:12px;color:var(--txt3,#8b93a1)">' + osIcon('loader') + ' <b>Durante el hold</b> (antes del refi, si ya renta): pagando el HML ' + UW_M(o.intereses.intMensualHarmony) + '/mes (⛓ Calc 4) en vez de la cuota DSCR → <b style="color:' + (g.flujo + g.pagoDscr - o.intereses.intMensualHarmony >= 0 ? 'var(--pos,#4ade9e)' : 'var(--neg,#ff6b6b)') + '">' + UW_M(g.flujo + g.pagoDscr - o.intereses.intMensualHarmony) + '/mes</b></div>' : '');
  return '<div style="max-width:560px;margin:0 auto">'
    + '<div style="margin-bottom:16px"><div style="font-size:19px;font-weight:700">Ingreso Mensual</div><div style="font-size:13px;color:var(--txt3,#8b93a1)">El flujo que deja la casa rentada, después de TODOS los gastos.</div></div>'
...
function ffUwViewVenta() {
  ...
  const col = pos ? 'var(--pos,#4ade9e)' : 'var(--neg,#ff6b6b)';
  ...
  const semColor = v.regla70 === false ? 'var(--neg,#ff6b6b)' : (v.regla70 && pos ? 'var(--pos,#4ade9e)' : 'var(--amber,#fbbf24)');
  ...
  const inputs = '<div class="card" style="padding:20px 22px;margin-bottom:16px;border-radius:18px">'
    ...
    + kitInputSm('Payoff del HML ($)', v.payoff, "ffUwSet('venta_payoff',VAL)", { foot: 'fuente: ' + v.payoffFuente + (+inp.venta_payoff > 0 ? ' · <a style="cursor:pointer;color:var(--a1,#5c79f0)" onclick="ffUwSet(\'venta_payoff\',0)">↩ auto</a>' : '') })
  ...
  const cascada = '<div class="card" style="padding:20px 22px;border-radius:18px">'
    + '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1);margin-bottom:4px">Utilidad del proyecto (para decidir)</div>'
    ...
    + '<div style="font-size:10.5px;color:var(--txt3,#8b93a1);margin-top:6px">El <b>payoff del HML</b> no entra acá: el préstamo es financiación del all-in. La utilidad usa los costos reales del proyecto.</div>'
    + '</div>';
  ...
  const metricas = '<div class="card" style="padding:20px 22px;border-radius:18px;margin-top:14px">'
    + '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1);margin-bottom:4px">En la mesa de cierre (Net Wire)</div>'
    + kitRow('Precio ' + UW_M(v.precio) + ' &minus; costos de venta ' + UW_M(v.costosVenta) + ' &minus; payoff HML ' + UW_M(v.payoff), null, { txt: UW_M2(v.netWire), color: v.netWire >= 0 ? 'var(--pos,#4ade9e)' : 'var(--neg,#ff6b6b)' })
    + '<div style="border-top:1px solid var(--line,rgba(255,255,255,.12));margin-top:10px;padding-top:12px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1)">Métricas del flip</div>'
    ...
    + kitRow('Check regla 70/75% (all-in ≤ MAO)', null, { txt: v.regla70 == null ? '—' : (v.regla70 ? '✓ cumple' : '✗ no cumple'), color: v.regla70 === false ? 'var(--neg,#ff6b6b)' : v.regla70 ? 'var(--pos,#4ade9e)' : 'inherit', last: true })
    + '<div style="border-top:1px solid var(--line,rgba(255,255,255,.12));margin-top:10px;padding-top:12px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:var(--txt3,#8b93a1)">Reparto (devolver capital + split equity)</div>'
    + kitRow('↩ Capital devuelto a inversionistas', v.capital)
    + kitRow('Inversionista (' + v.splitInvPct + '%)', v.parteInv, { color: 'var(--a2,#3a5be0)' })
    + kitRow('Operador (' + (100 - v.splitInvPct) + '%)', v.parteOp, { last: true })
    + '</div>';
  return '<div style="max-width:560px;margin:0 auto">'
    + '<div style="margin-bottom:16px"><div style="font-size:19px;font-weight:700">Venta (fix &amp; flip)</div><div style="font-size:13px;color:var(--txt3,#8b93a1)">Vendés al ARV: la utilidad real del proyecto (costos, no el préstamo), el ROI anualizado y el reparto.</div></div>'
...
function ffUwViewUnificada() {
  ...
  return '<div class="card" style="text-align:center;padding:24px;border:2px solid ' + verColor + '"><div style="font-size:12px;color:var(--txt2,#c7cdd8);font-weight:600;text-transform:uppercase;letter-spacing:.5px">Veredicto del deal</div><div style="font-size:46px;font-weight:800;color:' + verColor + '">' + u.veredicto + '</div><div style="display:flex;gap:8px;justify-content:center;margin-top:10px;flex-wrap:wrap">' + chip(u.gAllIn, 'all-in &le;' + u.allInMax + '% ARV') + chip(u.gDeficit, 'sin riesgo déficit') + chip(u.gFlujo, 'flujo +') + '</div><button class="repbtn" style="margin-top:16px" onclick="ffUwPresentacion()">&#128196; Generar presentación de negocio</button></div>' +
...
function ffUwPresentacion() {
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Análisis ${UW_E(a.nombre)}</title><style>
    ...
    .h{display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #3a5be0;padding-bottom:14px}
    .logo{font-size:22px;font-weight:800;color:#3a5be0}.logo span{display:block;font-size:10px;letter-spacing:2px;color:#666}
    ...
    .chain{font-size:12px;color:#444;line-height:1.6;margin-top:16px;background:#faf8f4;padding:12px;border-radius:8px}
    .btn{position:fixed;top:10px;right:10px;padding:8px 16px;background:#3a5be0;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:700}
    @media print{.btn{display:none}}</style></head><body>
    <button class="btn" onclick="window.print()">${osIcon('printer')} PDF</button>
...
function ffUwRcCompsBox() {
  ...
  if (s === 'loading') return '<div style="font-size:11px;color:var(--txt3,#8b93a1)">' + osIcon('loader') + ' consultando mercado en vivo…</div>';
  if (!s) return '<button class="repbtn ghost" style="padding:5px 11px;font-size:11px" onclick="ffUwRentcast(\'value\')">' + osIcon('link') + ' Traer mercado en vivo (RentCast)</button>';
  if (s.disponible === false) return '<div style="font-size:11px;color:var(--amber,#fbbf24)">' + osIcon('alert') + ' Mercado en vivo no disponible' + (s.error ? ' (' + UW_E(String(s.error).slice(0, 60)) + ')' : '') + ' — se usa el $/sqft por zona.</div>';
  const comps = (s.payload && s.payload.comparables) || [];
  const fecha = s.fetched_at ? String(s.fetched_at).slice(0, 10) : '';
  return '<div style="background:color-mix(in srgb, var(--a2,#3a5be0) 7%, transparent);border-radius:9px;padding:10px;margin-top:6px">'
    + '<div style="font-size:11px;font-weight:700;color:var(--a2,#3a5be0)">Mercado en vivo (RentCast) · referencia, NO alimenta cálculos ' + (s.cached ? '· cache' : '') + '</div>'
    ...
    + '<div style="font-size:10px;color:var(--txt3,#8b93a1)">' + comps.length + ' comparables' + (fecha ? ' · ' + fecha : '') + '</div>'
...
function ffUwRcRentBox() {
  ...
  if (s === 'loading') return '<div style="font-size:11px;color:var(--txt3,#8b93a1)">' + osIcon('loader') + ' consultando renta de mercado…</div>';
  if (!s) return '<button class="repbtn ghost" style="padding:5px 11px;font-size:11px" onclick="ffUwRentcast(\'rent\')">' + osIcon('link') + ' Sugerir renta (RentCast)</button>';
  if (s.disponible === false) return '<div style="font-size:11px;color:var(--amber,#fbbf24)">' + osIcon('alert') + ' Renta de mercado no disponible — usá tu proyección.</div>';
  return '<div style="background:rgba(52,211,153,.06);border-radius:9px;padding:10px;margin-top:6px"><div style="font-size:11px;font-weight:700;color:var(--pos,#4ade9e)">Renta de mercado (RentCast) · sugerencia</div><div style="display:flex;align-items:center;gap:10px;margin-top:4px"><div style="font-size:22px;font-weight:800">' + UW_M(s.value) + '/mes</div><button class="repbtn" style="padding:4px 12px;font-size:11px" onclick="ffUwSet(\'renta_mensual\',' + (+s.value || 0) + ')">Usar esta renta</button></div>' + ((s.payload && s.payload.comparables) ? '<div style="font-size:10px;color:var(--txt3,#8b93a1);margin-top:4px">' + s.payload.comparables.length + ' comparables de renta</div>' : '') + '<button class="repbtn ghost" style="padding:3px 9px;font-size:10px;margin-top:6px" onclick="ffUwRentcast(\'rent\',true)">↻ Actualizar</button></div>';
}
