...
(function (root) {
  ...
  function render() {
    ...
    const styles = '<style>'
      + '#ff-overlay .lds-shell{max-width:1180px;margin:0 auto;padding-bottom:28px}'
      + '#ff-overlay .lds-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:18px}'
      + '#ff-overlay .lds-kpis>.card{min-width:0;background:linear-gradient(145deg,color-mix(in srgb,var(--a1,#5c79f0) 7%,var(--card,#111827)),var(--card,#111827));overflow:hidden}'
      + '#ff-overlay .lds-kpis .big{font-size:clamp(24px,2.4vw,34px);white-space:nowrap}'
      + '#ff-overlay .lds-split{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(280px,.85fr);gap:14px;align-items:start;margin-top:14px}'
      + '#ff-overlay .lds-finance{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:14px}'
      + '#ff-overlay .lds-finance .card,#ff-overlay .lds-split .card{min-width:0}'
      + '@media(max-width:980px){#ff-overlay .lds-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}#ff-overlay .lds-split{grid-template-columns:1fr}}'
      + '@media(max-width:620px){#ff-overlay .lds-kpis,#ff-overlay .lds-finance{grid-template-columns:1fr}#ff-overlay .lds-shell{padding:0 2px 24px}}'
      + '</style>';
    return styles + '<div class="lds-shell">'
