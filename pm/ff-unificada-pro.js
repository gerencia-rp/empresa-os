...
function unTimeline(c) {
  ...
  const pasos = u.modo === 'venta'
    ? [
      [osIcon('key'), 'Compra', c.inp.purchase > 0 ? UN_M(+c.inp.purchase) : '—'],
      [osIcon('hammer'), 'Obra', c.mesesObra ? c.mesesObra + ' meses' : '—'],
      [osIcon('landmark'), 'Venta', c.o.venta.precio > 0 ? UN_M(c.o.venta.precio) : '—'],
      [osIcon('dollar'), 'Utilidad', c.o.venta.utilidad != null ? UN_M(c.o.venta.utilidad) : '—'],
    ]
    : [
      [osIcon('key'), 'Compra', c.inp.purchase > 0 ? UN_M(+c.inp.purchase) : '—'],
      [osIcon('hammer'), 'Obra', c.mesesObra ? c.mesesObra + ' meses' : '—'],
      [osIcon('house'), 'Renta', c.mesesHastaRenta ? 'mes ' + c.mesesHastaRenta : '—'],
      [osIcon('landmark'), 'Refi', c.o.cashout.prestamoRefi > 0 ? UN_M(c.o.cashout.prestamoRefi) : '—'],
      [osIcon('dollar'), 'Capital recuperado', rec != null ? (rec >= 100 ? '♾️ ' + rec + '%' : rec + '%') : '—'],
    ];
...
function ffUnificadaView() {
  ...
  const banda = '<div class="ap-card" style="padding:20px 24px;display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;border-color:' + verColor + ';box-shadow:0 0 0 1px ' + verColor + ',0 6px 22px rgba(23,43,77,.08)">'
    ...
    + '<button class="ap-btn" onclick="ffUwPresentacion()">' + osIcon('file') + ' Generar presentación / Imprimir PDF</button></div>';
  ...
  return '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px"><b style="font-size:18px">' + osIcon('target') + ' El deal completo de un vistazo</b><span class="ap-pill">' + (esVenta ? 'estrategia: vender (flip)' : 'estrategia: rentar (hold)') + ' — mismos números de las calculadoras</span></div>'
...
function ffUnificadaOnePager() {
  ...
  const pasos = esVenta
    ? [[osIcon('key'), 'Compra'], [osIcon('hammer'), 'Obra ' + (c.mesesObra ? c.mesesObra + 'm' : '')], [osIcon('landmark'), 'Venta ' + M(o.venta.precio)], [osIcon('dollar'), o.venta.utilidad != null ? 'Utilidad ' + M(o.venta.utilidad) : 'Utilidad']]
    : [[osIcon('key'), 'Compra'], [osIcon('hammer'), 'Obra ' + (c.mesesObra ? c.mesesObra + 'm' : '')], [osIcon('house'), 'Renta' + (c.mesesHastaRenta ? ' mes ' + c.mesesHastaRenta : '')], [osIcon('landmark'), 'Refi'], [osIcon('dollar'), u.recuperaPct != null ? (u.recuperaPct >= 100 ? '♾️ ' : '') + u.recuperaPct + '% recuperado' : 'Recuperación']];
  ...
  const html = '<!doctype html><html><head><meta charset="utf-8"><title>' + E(a.direccion || a.nombre) + ' · Rental Profits</title><style>'
    ...
    + (esVenta
      ? sec('Utilidad del flip',
        ...
        + sec(osIcon('trophy') + ' Retorno y reparto',
