-- Certificación ejecutiva diaria para una ausencia prolongada.
-- Solo observa y reporta: no cambia datos operativos ni aprueba decisiones.
...
declare
  ...
  v_photos int:=0; v_cron_failed int:=0; v_passed int:=0; v_out jsonb;
begin
  ...
  v_out:=jsonb_build_object('corte',v_corte,'compuertas_aprobadas',v_passed,'compuertas_totales',8,
    'estado',case when v_passed=8 then 'listo' else 'no_listo' end,
    'compuertas',jsonb_build_object(
      ...
      'automatizaciones',jsonb_build_object('ok',v_cron_failed=0,'fallos_7d',v_cron_failed)),
    'bloqueo_principal',case
      ...
      when v_passed<8 then 'Resolver las compuertas técnicas pendientes.' else 'Ninguno.' end,
...
do $$ begin
  perform cron.unschedule(jobname) from cron.job where jobname='continuity-absence-readiness';
exception when others then null; end $$;
select cron.schedule('continuity-absence-readiness','45 12 * * *',
  $$select public.run_absence_readiness_review()$$);
comment on function public.run_absence_readiness_review() is
  'Certifica diariamente 8 compuertas para ausencia prolongada. Solo observa y reporta.';
