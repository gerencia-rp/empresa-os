-- v6: PRECIO FIJO = "Valor Remodelación" (valor_interno) confirmado CEO; tentativo si aún es la fórmula ×1.05.
...
SELECT
  ...
  pr.done, pr.total, pr.tareas_sin_map,
  round(coalesce(pr.avance_real, 0)) pct_tecnico,          -- avance TÉCNICO (ponderado si hay pesos)
  pr.metodo,
  coalesce(fp.presup_pronostico, pc.presupuesto) presupuesto, fp.presup_pronostico, (CASE WHEN fp.presup_pronostico IS NOT NULL THEN 'pronostico' WHEN pc.presupuesto IS NOT NULL THEN 'airtable' END) fuente_presupuesto, round(coalesce(pc.total_real, 0)) gasto_real,
  CASE WHEN coalesce(fp.presup_pronostico, pc.presupuesto) > 0 THEN round(100 * coalesce(pc.total_real, 0) / coalesce(fp.presup_pronostico, pc.presupuesto)) END pct_financiero,                            -- avance FINANCIERO = gasto/presupuesto (C2)
  ...
  r.valor_interno valor_remodelacion,   -- "Valor Remodelación" (precio fijo Structure One)
  CASE WHEN r.valor_interno > 0 AND abs(r.valor_interno - 1.05 * (coalesce(r.gasto_materiales,0) + coalesce(r.gasto_trabajadores,0))) / r.valor_interno < 0.02
    THEN true ELSE false END precio_tentativo,   -- aún es la fórmula costo×1.05: falta cargar el precio real
  coalesce(cm.modo, 'fijo') modo_cobro,
  CASE WHEN coalesce(cm.modo, 'fijo') = 'costplus'
    THEN round(1.05 * (CASE WHEN coalesce(pr.avance_real, 0) >= 5 AND coalesce(pc.total_real, 0) > 0 THEN round(pc.total_real / (pr.avance_real / 100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, 0) END))
    ELSE r.valor_interno END valor_cobro_proyectado,
  ...
  CASE WHEN coalesce(cm.modo,'fijo')='costplus' OR r.valor_interno > 0 THEN
    (CASE WHEN coalesce(cm.modo,'fijo')='costplus' THEN round(1.05 * (CASE WHEN coalesce(pr.avance_real,0) >= 5 AND coalesce(pc.total_real,0) > 0 THEN round(pc.total_real/(pr.avance_real/100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, 0) END)) ELSE r.valor_interno END) - (CASE WHEN coalesce(pr.avance_real, 0) >= 5 AND coalesce(pc.total_real, 0) > 0 THEN round(pc.total_real / (pr.avance_real / 100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, 0) END)
    ELSE NULL END ganancia_proyectada,
  CASE WHEN coalesce(pc.total_real, 0) > 0 AND (r.valor_interno > 0 OR coalesce(cm.modo,'fijo')='costplus') THEN
    round(100.0 * ((CASE WHEN coalesce(cm.modo,'fijo')='costplus' THEN round(1.05 * (CASE WHEN coalesce(pr.avance_real,0) >= 5 THEN round(pc.total_real/(pr.avance_real/100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, pc.total_real) END)) ELSE r.valor_interno END) - (CASE WHEN coalesce(pr.avance_real, 0) >= 5 THEN round(pc.total_real / (pr.avance_real / 100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, pc.total_real) END)) / nullif(pc.total_real, 0))
  ...
  CASE WHEN coalesce(cm.modo,'fijo')='costplus' THEN 'verde'
    WHEN r.valor_interno > 0 AND coalesce(pr.avance_real, 0) >= 5 AND coalesce(pc.total_real, 0) > 0
    AND r.valor_interno < round(pc.total_real / (pr.avance_real / 100.0)) THEN 'rojo'
    WHEN r.valor_interno > 0 THEN 'verde' ELSE 'gris' END sem_ganancia
...
LEFT JOIN (SELECT p2.property_id, sum((e2->>'total')::numeric) presup_pronostico FROM public.remodel_projects p2, jsonb_array_elements(p2.activities) e2 GROUP BY p2.property_id) fp ON fp.property_id = r.property_id
LEFT JOIN public.remodel_cobro_modo cm ON cm.property_id = r.property_id AND cm.active
