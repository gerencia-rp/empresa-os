-- Punto 4: definición de cobro por obra — PRECIO FIJO (monto_real, puede dar pérdida) vs COST-PLUS (costo×1.05).
CREATE TABLE IF NOT EXISTS public.remodel_cobro_modo (
  property_id uuid PRIMARY KEY,
  modo text DEFAULT 'fijo' CHECK (modo IN ('fijo','costplus')),
  updated_at timestamptz DEFAULT now(), updated_by text,
  active boolean DEFAULT true, archived_at timestamptz
);
ALTER TABLE public.remodel_cobro_modo ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS rcm_rw ON public.remodel_cobro_modo;
CREATE POLICY rcm_rw ON public.remodel_cobro_modo FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
SELECT
  ...
  CASE WHEN coalesce(cm.modo, 'fijo') = 'costplus'
    ...
    ELSE r.monto_real END valor_cobro_proyectado,
  ...
  CASE WHEN coalesce(cm.modo,'fijo')='costplus' OR r.monto_real > 0 THEN
    (CASE WHEN coalesce(cm.modo,'fijo')='costplus' THEN round(1.05 * (CASE WHEN coalesce(pr.avance_real,0) >= 5 AND coalesce(pc.total_real,0) > 0 THEN round(pc.total_real/(pr.avance_real/100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, 0) END)) ELSE r.monto_real END) - (CASE WHEN coalesce(pr.avance_real, 0) >= 5 AND coalesce(pc.total_real, 0) > 0 THEN round(pc.total_real / (pr.avance_real / 100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, 0) END)
    ELSE NULL END ganancia_proyectada,
  CASE WHEN coalesce(pc.total_real, 0) > 0 AND (r.monto_real > 0 OR coalesce(cm.modo,'fijo')='costplus') THEN
    round(100.0 * ((CASE WHEN coalesce(cm.modo,'fijo')='costplus' THEN round(1.05 * (CASE WHEN coalesce(pr.avance_real,0) >= 5 THEN round(pc.total_real/(pr.avance_real/100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, pc.total_real) END)) ELSE r.monto_real END) - (CASE WHEN coalesce(pr.avance_real, 0) >= 5 THEN round(pc.total_real / (pr.avance_real / 100.0)) ELSE coalesce(fp.presup_pronostico, pc.presupuesto, pc.total_real) END)) / nullif(pc.total_real, 0))
  ...
  CASE WHEN coalesce(cm.modo,'fijo')='costplus' THEN 'verde'
    WHEN r.monto_real > 0 AND coalesce(pr.avance_real, 0) >= 5 AND coalesce(pc.total_real, 0) > 0
