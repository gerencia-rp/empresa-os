// get-insights · devuelve el último batch de insights (no leído primero).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await db.from("insights").select("id, computed_at, insights, read").order("computed_at", { ascending: false }).limit(1);
    if (error) return corsResponse(req, { error: error.message }, 500);
    return corsResponse(req, { latest: (data && data[0]) || null });
  } catch (e) {
